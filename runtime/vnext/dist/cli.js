...
var CLAIM_EVIDENCE_COMPLETION_RULE = "non-empty acceptance-bearing frozen plan; due slots require bound successful reports and applicable subject revisions; close checks all slots and consumed prerequisites";
...
var REVIEW_BLOCKER_ROUTES = ["review-change", "debug-task", "prepare-task:replan", "user"];
...
function captureReviewTarget(root, paths) {
  ...
  const entries = normalizedPaths.map((relativePath) => {
    ...
    if (filePath !== resolvedRoot && !filePath.startsWith(`${resolvedRoot}${path4.sep}`)) {
    ...
    if (stat.isSymbolicLink()) {
      return { path: relativePath, state: "symlink", sha256: sha2562(fs3.readlinkSync(filePath)) };
    }
    if (!stat.isFile()) {
      fail2("REVIEW_TARGET_INVALID", `review target must be a file, symlink, or absent path: ${relativePath}`);
    }
    return { path: relativePath, state: "file", sha256: sha2562(fs3.readFileSync(filePath)) };
  });
  return {
    ...
    const attempts = ledger.attempts.map((raw2) => {
      ...
      if (item.blocker !== null) {
        ...
        if (blocker.execution_result.outcome !== "blocked")
          fail2("RETRY_LEDGER_INVALID", "Retained failure must be blocked.");
      }
...
function validateRetryResolution(root, current, delta, failure) {
  ...
  for (const ref of delta.blocker_resolution_refs) {
    ...
    if (!fs3.existsSync(absolute) || !fs3.statSync(absolute).isFile() || fs3.statSync(absolute).size > 65536)
      fail2("RETRY_RESOLUTION_REQUIRED", "Resolution must be a retained bounded JSON report.");
    let parsed;
    try {
      parsed = JSON.parse(fs3.readFileSync(absolute, "utf8"));
    } catch {
      fail2("RETRY_RESOLUTION_REQUIRED", "Resolution must be structured JSON, not a free-form unlock note.");
    }
    const report = expectRecord2(parsed, "retry resolution");
    expectExactKeys2(report, ["kind", "task_id", "document_id", "step_id", "blocked_attempt_id", "evidence_plan_revision", "subject_revision", "status", "diagnosis", "resolution"], "retry resolution");
    if (report.kind !== "environment-restored/v1" || report.status !== "passed" || report.task_id !== current.runtimeState.task_id || report.document_id !== current.sourceTuple.document_id || report.step_id !== delta.step_id || report.blocked_attempt_id !== delta.blocked_attempt_id || report.evidence_plan_revision !== current.runtimeState.evidence_plan_revision || report.subject_revision !== failure.subject_snapshot.revision)
      fail2("RETRY_RESOLUTION_REQUIRED", "Resolution report must bind this exact environment failure, plan and subjects.");
    expectText(report.diagnosis, "retry diagnosis");
    expectText(report.resolution, "retry resolution");
  }
}
function emptyReviewCoverage(planRevision) {
  return {
...
function validateReviewCoverage(value) {
  ...
  const preimages = source.preimages.map((value2, index) => {
    ...
    if (digest({ path: item.path, state: item.state, sha256: item.sha256 }) !== digest(entry))
      fail2("REVIEW_COVERAGE_INVALID", "preimage identity differs from base.");
    const content = item.content_base64;
    if (entry.state === "absent") {
      if (content !== null)
        fail2("REVIEW_COVERAGE_INVALID", "absent preimage has content.");
    } else {
      if (typeof content !== "string")
        fail2("REVIEW_COVERAGE_INVALID", "preimage content is required.");
      const buffer = Buffer.from(content, "base64");
      if (buffer.toString("base64") !== content || sha2562(buffer) !== entry.sha256)
        fail2("REVIEW_COVERAGE_INVALID", "preimage content hash is invalid.");
    }
    return { ...entry, content_base64: content };
  });
...
function registerReviewCoverage(root, current, paths) {
  ...
  const preimages = [...old?.preimages ?? [], ...added.entries.map((entry) => {
    const absolute = path4.resolve(root, entry.path);
    const content = entry.state === "absent" ? null : entry.state === "symlink" ? Buffer.from(fs3.readlinkSync(absolute)) : fs3.readFileSync(absolute);
...
function cumulativeReviewExecution(current, execution) {
  ...
  if (!coverage || !execution.execution_result)
    return execution;
  return {
    ...execution,
...
function validateTestAssessment(value) {
  ...
  if (typeof source.applicable !== "boolean")
    fail2("REVIEW_ASSESSMENT_REQUIRED", "applicable must be boolean.");
...
function createReviewChangeDelta(base, target) {
  ...
  if (digest(basePaths) !== digest(targetPaths)) {
    fail2("REVIEW_TARGET_PATH_CONFLICT", "review base and target must contain the same canonical path set.");
  }
...
function validateVNextRuntimeContract(root, requireDependencies = false) {
  ...
  expectExactKeys2(proposal, ["schema_version", "kind", "caller", "operation_kinds", "source_tuple", "required_envelope", "finding_queue_admission", "finding_queue_repair", "task_state", "execute_step", "review_change", "prepare_task", "inbox_record", "lifecycle", "close_task", "lesson_marker"], "Runtime contract.proposal");
  ...
  expectSetEqual(expectStringArray2(proposal.caller, "Runtime contract.proposal.caller"), ["execute-step", "review-change", "prepare-task", "task-lifecycle", "capture-work-item", "close-task"], "Runtime contract proposal callers");
  ...
  expectSetEqual(expectStringArray2(stepProgressContract.optional, "Runtime contract.proposal.task_state.step_progress.optional", true), ["note", "repair_fingerprint", "repair_fingerprints", "repair_wave_id", "change_set_id", "review_receipt", "claim_evidence", "execution_result"], "Runtime contract task-state optional fields");
  ...
  expectExactKeys2(claimEvidenceContract, ["stored_in", "record_fields", "slot_fields", "complete_dispositions", "incomplete_dispositions", "completion_rule"], "Runtime contract.proposal.task_state.claim_evidence");
  if (claimEvidenceContract.stored_in !== "canonical CURRENT_TASK.runtime_state") {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime claim evidence must be stored in the canonical CURRENT_TASK runtime state.");
  }
  ...
  if (claimEvidenceContract.completion_rule !== CLAIM_EVIDENCE_COMPLETION_RULE) {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime claim evidence completion must require a non-empty frozen plan with an acceptance claim and every planned slot with its evidence_refs.");
  }
  ...
  if (claimEvidenceMigrationContract.mode !== "default" || claimEvidenceMigrationContract.action !== "migrate-claim-evidence") {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime claim evidence migration must use default/migrate-claim-evidence.");
  }
  expectSetEqual(expectStringArray2(claimEvidenceMigrationContract.required, "Runtime contract claim evidence migration required"), ["claim_evidence", "evidence_refs"], "Runtime contract claim evidence migration required fields");
  if (claimEvidenceMigrationContract.from !== "active + active legacy-or-empty-plan" || claimEvidenceMigrationContract.to !== "active + active") {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime claim evidence migration transition is invalid.");
  }
  if (claimEvidenceMigrationContract.mutation !== "install one non-empty acceptance-bearing frozen claim_evidence plan; do not change task semantics") {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime claim evidence migration mutation boundary is invalid.");
  }
  ...
  expectSetEqual(expectStringArray2(reviewReceiptContract.required, "Runtime contract.proposal.task_state.review_receipt.required"), ["cycle_id", "cycle_phase", "change_set_id", "review_target_revision", "verdict", "admitted_fingerprints", "evidence_refs"], "Runtime contract review receipt required fields");
  ...
  if (reviewReceiptContract.target_verification !== "runtime-file-manifest")
    fail2("RUNTIME_CONTRACT_INVALID", "review receipt target verification must be runtime-file-manifest.");
  ...
  if (reviewResultContract.stored_in !== "canonical CURRENT_TASK.runtime_state.pending_review_result")
    fail2("RUNTIME_CONTRACT_INVALID", "review result must use canonical pending review storage.");
  ...
  if (reviewResultConsumers.clean !== "execute-step:complete-reviewed-step" || reviewResultConsumers.findings !== "execute-step:begin-repair" || reviewResultConsumers.blocked !== "caller-route")
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime review-result consumers are invalid.");
  ...
  if (draftContract.claim_evidence !== DRAFT_CLAIM_EVIDENCE_REQUIREMENT) {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime draft claim evidence requirement is invalid.");
  }
  ...
  expectSetEqual(expectStringArray2(draftTaskBasis.required, "Runtime contract draft task basis required fields"), ["original_request", "user_decisions"], "Runtime contract draft task basis required fields");
  expectSetEqual(expectStringArray2(draftTaskBasis.source_fields, "Runtime contract draft task basis source fields"), ["source", "verbatim"], "Runtime contract draft task basis source fields");
  if (draftTaskBasis.storage !== "identity-derived-linked-document" || draftTaskBasis.write_binding !== "same-atomic-draft-transaction" || draftTaskBasis.review_results_forbidden !== true) {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime draft task basis storage boundary is invalid.");
  }
  ...
  if (decisionPartition.decided !== "confirmed_decisions" || decisionPartition.unresolved !== "open_questions") {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime prepare-task adapter must preserve decided and unresolved design choices separately.");
  }
  ...
  if (commandFootprintPreflight.source !== "implementation_steps[].commands" || commandFootprintPreflight.evaluator !== "shared-mutation-scope-evaluator" || commandFootprintPreflight.timing !== "before-draft-commit") {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime prepare-task adapter command-footprint preflight is not bound to declared step commands before draft commit.");
  }
  expectSetEqual(expectStringArray2(commandFootprintPreflight.fields, "Runtime contract.proposal.prepare_task.semantic_adapter.command_footprint_preflight.fields"), ["command", "expected_repo_writes"], "Runtime prepare-task adapter command-footprint fields");
  ...
  expectSetEqual(expectStringArray2(testStrategy.modes, "Runtime contract prepare-task test-strategy modes"), ["flexible", "test-first", "implementation-first", "not-applicable"], "Runtime contract prepare-task test-strategy modes");
  ...
  if (prepareTaskContract.migration_mode !== "default")
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime contract prepare-task migration_mode must be default.");
  expectSetEqual(expectStringArray2(prepareTaskContract.migration_actions, "Runtime contract prepare-task migration_actions"), [...CLAIM_EVIDENCE_MIGRATION_ACTIONS], "Runtime contract prepare-task migration actions");
  ...
  if (closeTaskContract.claim_evidence !== CLOSE_TASK_CLAIM_EVIDENCE_RULE) {
    fail2("RUNTIME_CONTRACT_INVALID", "Runtime close-task claim evidence derivation rule is invalid.");
  }
...
function validateExecutionResultPaths(value, location) {
  const values = expectStringArray2(value, location, true, MAX_EXECUTION_RESULT_ITEMS).map((item, index) => normalizeRepoPath2(item, `${location}[${index}]`));
  if (new Set(values).size !== values.length)
    fail2("RUNTIME_SCHEMA_INVALID", `${location} must not contain duplicates.`);
  return values;
}
function validateReviewTarget(value, location) {
  const target = expectRecord2(value, location);
  expectExactKeys2(target, ["kind", "revision", "entries"], location);
  if (target.kind !== "runtime-file-manifest/v1") {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.kind must be runtime-file-manifest/v1.`);
  }
  ...
  const entries = target.entries.map((value2, index) => {
    ...
    if (state === "absent" !== (entryDigest === null)) {
      fail2("RUNTIME_SCHEMA_INVALID", `${entryLocation}.sha256 must be null exactly when state is absent.`);
    }
    if (entryDigest !== null && !/^[a-f0-9]{64}$/u.test(entryDigest)) {
      fail2("RUNTIME_SCHEMA_INVALID", `${entryLocation}.sha256 must be SHA-256.`);
    }
    return {
      path: normalizeRepoPath2(expectString2(entry.path, `${entryLocation}.path`), `${entryLocation}.path`),
      state,
      sha256: entryDigest
    };
  });
  ...
  if (new Set(sortedPaths).size !== sortedPaths.length || entries.some((item, index) => item.path !== sortedPaths[index])) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.entries must use unique paths in canonical sort order.`);
  }
  const revision = expectString2(target.revision, `${location}.revision`);
  if (!/^[a-f0-9]{64}$/u.test(revision) || revision !== digest({ kind: "runtime-file-manifest/v1", entries })) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.revision does not match its canonical file manifest.`);
  }
  return { kind: "runtime-file-manifest/v1", revision, entries };
}
function validateRuntimeReviewTarget(value, location = "review_target") {
  return validateReviewTarget(value, location);
}
function validateReviewChangeDelta(value, location, base, target) {
  const delta = expectRecord2(value, location);
  expectExactKeys2(delta, ["kind", "base_revision", "target_revision", "entries"], location);
  if (delta.kind !== "runtime-file-delta/v1") {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.kind must be runtime-file-delta/v1.`);
  }
  ...
  const entries = delta.entries.map((value2, index) => {
    ...
    if (beforeState === "absent" !== (beforeDigest === null) || afterState === "absent" !== (afterDigest === null)) {
      fail2("RUNTIME_SCHEMA_INVALID", `${entryLocation} digests must be null exactly for absent states.`);
    }
    if (beforeDigest !== null && !/^[a-f0-9]{64}$/u.test(beforeDigest) || afterDigest !== null && !/^[a-f0-9]{64}$/u.test(afterDigest)) {
      fail2("RUNTIME_SCHEMA_INVALID", `${entryLocation} digests must be SHA-256.`);
    }
    return {
      path: normalizeRepoPath2(expectString2(entry.path, `${entryLocation}.path`), `${entryLocation}.path`),
  ...
  const baseRevision = expectString2(delta.base_revision, `${location}.base_revision`, /^[a-f0-9]{64}$/u);
  const targetRevision = expectString2(delta.target_revision, `${location}.target_revision`, /^[a-f0-9]{64}$/u);
  ...
  if (digest(normalized) !== digest(expected)) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location} does not match the canonical before/after manifests.`);
  }
  return normalized;
}
...
function validateStepExecutionResult(value, location) {
  ...
  if (digest([...actualChangedPaths].sort()) !== digest(changeDelta.entries.map((item) => item.path))) {
    fail2("RUNTIME_STATE_CONFLICT", `${location}.actual_changed_paths must exactly match the Runtime before/after delta.`);
  }
  const commandValues = result.command_results;
  if (!Array.isArray(commandValues) || commandValues.length > MAX_EXECUTION_RESULT_ITEMS) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.command_results must be a bounded array.`);
  }
  ...
  if (new Set(commandResults.map((item) => item.command)).size !== commandResults.length) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.command_results must not contain duplicate commands.`);
  }
  ...
  if (!Array.isArray(validationValues) || validationValues.length > MAX_EXECUTION_RESULT_ITEMS) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.validation_results must be a bounded array.`);
  }
  ...
  if (new Set(validationResults.map((item) => item.validation)).size !== validationResults.length) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.validation_results must not contain duplicate validations.`);
  }
  const acceptanceEvidence = Array.isArray(result.acceptance_evidence) && result.acceptance_evidence.every((item) => isRecord2(item) && ("acceptance" in item)) ? result.acceptance_evidence.map((item) => {
  ...
  if (outcome === "implemented" && (blocker !== null || statuses.some((status) => status !== "passed" && status !== "expected-failure"))) {
    fail2("RUNTIME_STATE_CONFLICT", `${location}.implemented requires every planned result to pass and no blocker.`);
  }
  ...
  return {
    ...result.attempt_id === undefined ? {} : { attempt_id: expectString2(result.attempt_id, "execution_result.attempt_id", SAFE_KEY_PATTERN2) },
...
function validateStepAcceptanceEvidence(value, location) {
  if (!Array.isArray(value) || value.length > MAX_EXECUTION_RESULT_ITEMS)
    fail2("RUNTIME_SCHEMA_INVALID", `${location} must be a bounded array.`);
  const results = value.map((raw) => {
    const item = expectRecord2(raw, location);
    expectExactKeys2(item, ["claim_id", "slot_id", "check_id", "minimum_type", "disposition", "evidence_refs", "report"], location);
    return {
  ...
  if (new Set(results.map((item) => `${item.claim_id}/${item.slot_id}`)).size !== results.length)
    fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", "duplicate slot results.");
  return results;
}
...
function evidenceSlotDefinition(slot) {
...
function assertEvidencePlan(definition, records, fresh = false) {
  ...
  if (digest(acceptance) !== digest(projected))
  ...
  for (const record of records) {
    ...
    for (const slot of record.slots) {
      ...
      if (ids.has(check.check_id))
        fail2("CLAIM_EVIDENCE_PLAN_INVALID", "check_id must be unique across the task.");
      ...
      if (slot.applicability === "before-step") {
        if (!steps.includes(slot.before_step_id) || steps.indexOf(slot.due_step_id) > steps.indexOf(slot.before_step_id))
          fail2("CLAIM_EVIDENCE_PLAN_INVALID", "prerequisite must be due no later than its constrained step.");
        if (slot.before_step_id === steps[0])
          fail2("TEST_STRATEGY_PREREQUISITE_UNSUPPORTED", "A first-step prerequisite has no earlier authorized result submission step.");
      } else if (slot.before_step_id !== undefined || slot.prerequisite_receipt !== undefined)
        fail2("CLAIM_EVIDENCE_PLAN_INVALID", "current slots cannot carry prerequisite fields.");
      if (fresh && (slot.report || slot.prerequisite_receipt || slot.disposition !== "missing" || slot.evidence_refs.length))
        fail2("CLAIM_EVIDENCE_PLAN_INVALID", "new plans cannot import results or prerequisite receipts.");
    }
  }
  ...
  if (fresh && parsedSteps.some((step) => step.review_checkpoint === "not-required")) {
    if (parsedSteps.some((step) => !step.checkpoint_boundary))
      fail2("REVIEW_CHECKPOINT_REQUIRED", "Sparse checkpoints require explicit reasons on every step.");
    const final = parsedSteps.at(-1);
    if (final.review_checkpoint !== "required" && !(final.checkpoint_boundary?.startsWith("final-exemption:") && final.checkpoint_boundary.slice("final-exemption:".length).trim()))
      fail2("REVIEW_CHECKPOINT_REQUIRED", "A final waiver must explicitly cover cumulative changes with final-exemption: and a reason.");
    const earlier = new Set;
    for (const step of parsedSteps) {
      const scope = (step.mutation_scope ?? "").split(",").map((p) => p.trim().replace(/^`|`$/g, ""));
      if (step.review_checkpoint === "required" && [...earlier].some((p) => !scope.includes(p)))
        fail2("REVIEW_REPAIR_SCOPE_REQUIRED", "Required checkpoints must reserve exact earlier paths for repair.");
...
function assertPersistentTestAdmission(definition, records) {
...
function assertEvidenceReportApplicable(root, current, slot) {
  ...
  if (!report || !slot.check || report.actual_method !== slot.check.method || report.evidence_plan_revision !== current.runtimeState.evidence_plan_revision)
    fail2("CLAIM_EVIDENCE_INCOMPLETE", "report must bind its frozen check and plan revision.");
  if (report.actual_method === "human")
    fail2("EVIDENCE_AUTHORITY_UNSUPPORTED", "No authenticated human acceptance channel is bound; caller-reported human approval is insufficient.");
  if (!slot.evidence_refs.length || slot.evidence_refs.some((ref) => {
    try {
      const p = normalizeRepoPath2(ref.split("#")[0], "evidence_ref");
      return !fs3.statSync(path4.resolve(root, p)).isFile();
    } catch {
      return true;
    }
  }))
    fail2("EVIDENCE_ARTIFACT_UNAVAILABLE", "Each report needs retained repository-relative artifact files.");
  const snapshot = slot.prerequisite_receipt?.subject_snapshot ?? captureReviewTarget(root, slot.check.subject_paths);
  if (snapshot.revision !== report.subject_revision || digest(snapshot.entries.map((entry) => entry.path).sort()) !== digest([...slot.check.subject_paths].sort()))
    fail2("CLAIM_EVIDENCE_STALE", "evidence subject changed or does not match the frozen object set.");
...
function validateClaimEvidence(value, location) {
  ...
  const records = value.map((raw, index) => {
    ...
    const slots = record.slots.map((rawSlot, slotIndex) => {
      ...
      const minimumType = expectString2(slot.minimum_type, `${location}[${index}].slots[${slotIndex}].minimum_type`, CLAIM_EVIDENCE_TYPE_PATTERN);
      const disposition = expectEnum(slot.disposition, CLAIM_EVIDENCE_DISPOSITIONS, `${location}[${index}].slots[${slotIndex}].disposition`);
      const evidenceRefs = expectStringArray2(slot.evidence_refs, `${location}[${index}].slots[${slotIndex}].evidence_refs`, true, MAX_EVIDENCE_REFS);
      if (COMPLETE_CLAIM_EVIDENCE_DISPOSITIONS.includes(disposition) && evidenceRefs.length === 0) {
        fail2("CLAIM_EVIDENCE_INVALID", `${location}[${index}].slots[${slotIndex}] requires evidence_refs for disposition=${disposition}.`);
      }
      ...
      if (slot.check !== undefined) {
        ...
        expectExactKeys2(check, ["check_id", "method", "entry", "expected_observation", "required_boundaries", "allowed_substitutes", "subject_paths", "expected_result"], "slot.check");
        result.check = {
          ...
          subject_paths: expectStringArray2(check.subject_paths, "check.subject_paths", false, 256).map((p) => normalizeRepoPath2(p, "check.subject_paths")),
    ...
    if (new Set(slots.map((slot) => slot.slot_id)).size !== slots.length) {
      fail2("CLAIM_EVIDENCE_INVALID", `${location}[${index}].slots must have unique slot_id values.`);
    }
    return {
      claim_id: claimId,
...
function evaluateClaimEvidence(records, context) {
  ...
  try {
    ...
    if (revision !== current.runtimeState.evidence_plan_revision)
      return incomplete;
  } catch {
    return incomplete;
  }
...
function copyClaimEvidence(records) {
...
function requireClaimEvidencePlan(records, location, missingCode = "CLAIM_EVIDENCE_REQUIRED") {
  if (records === undefined || records.length === 0) {
    fail2(missingCode, `${location} must persist a non-empty claim_evidence plan before the task can proceed.`);
  }
  return [...records];
}
...
function assertClaimEvidencePlanPreserved(planned, proposed, location) {
  if (planned.length === 0) {
    fail2("CLAIM_EVIDENCE_REQUIRED", `${location} cannot initialize a claim_evidence plan during execution; prepare-task must persist it first.`);
  }
  ...
  for (const plannedRecord of planned) {
    ...
    if (!proposedRecord || proposedRecord.claim_kind !== plannedRecord.claim_kind || proposedRecord.requirement !== plannedRecord.requirement || proposedRecord.source_ref !== plannedRecord.source_ref) {
      fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", `${location} must preserve every planned claim identity and kind.`);
    }
    if (proposedRecord.slots.length !== plannedRecord.slots.length) {
      fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", `${location} cannot add or remove evidence slots for claim ${plannedRecord.claim_id}.`);
    }
    ...
    for (const plannedSlot of plannedRecord.slots) {
      ...
      if (!proposedSlot || digest(evidenceSlotDefinition(proposedSlot)) !== digest(evidenceSlotDefinition(plannedSlot))) {
        fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", `${location} must preserve evidence slot ${plannedRecord.claim_id}/${plannedSlot.slot_id} and its minimum type.`);
      }
      if (digest(proposedSlot.prerequisite_receipt ?? null) !== digest(plannedSlot.prerequisite_receipt ?? null) || "prerequisite_receipt" in proposedSlot !== "prerequisite_receipt" in plannedSlot || plannedSlot.prerequisite_receipt && digest(proposedSlot) !== digest(plannedSlot)) {
        fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", "Runtime prerequisite receipts and consumed evidence are immutable.");
      }
    }
  }
}
...
function validatePendingReviewResult(value, location, includeRecordedAt) {
  ...
  if (verdict === "findings" && (findings.length === 0 && unresolvedFingerprints.length === 0 || blocker !== null)) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location} findings result requires a finding and must not contain a blocker.`);
  }
...
function expectVerbatim(value, location, maxLength) {
  ...
  if (value.length > maxLength || /\0/u.test(value)) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location} must be at most ${maxLength} characters and contain no NUL byte.`);
  }
  return value;
}
function validateTaskBasisSource(value, location) {
  const source = expectRecord2(value, location);
  expectExactKeys2(source, ["source", "verbatim"], location);
  const locator = expectText(source.source, `${location}.source`, 1024);
  if (/[\r\n]/u.test(locator))
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.source must be one line.`);
  return {
    source: locator,
    verbatim: expectVerbatim(source.verbatim, `${location}.verbatim`, MAX_REPLAN_SECTION_CONTENT_LENGTH)
  };
}
function validateTaskBasis(value, location) {
  const basis = expectRecord2(value, location);
  expectExactKeys2(basis, ["original_request", "user_decisions"], location);
  if (!Array.isArray(basis.user_decisions) || basis.user_decisions.length > 64) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.user_decisions must be a bounded array.`);
  }
  const userDecisions = basis.user_decisions.map((item, index) => validateTaskBasisSource(item, `${location}.user_decisions[${index}]`));
  const decisionKeys = userDecisions.map((item) => `${item.source}\x00${item.verbatim}`);
  if (new Set(decisionKeys).size !== decisionKeys.length) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.user_decisions must not contain duplicate source excerpts.`);
  }
  return {
...
`).entries()) {
  ...
  if (strategy.source === "inferred-default") {
    const inferredMode = nonExecutable ? "not-applicable" : "flexible";
...
function assertBusinessEvidenceVersion(current) {
  if (current.runtimeState.business_evidence_version !== 1) {
    fail2("TASK_SEMANTICS_UPGRADE_REQUIRED", "This task predates business_evidence_version=1; use its previous installation or an explicitly authorized replan.");
  }
}
function executionPhaseForStrategy(strategy) {
...
function assertTestStrategySequenceReady(current, context = resolveTestStrategyExecutionContext(current)) {
  ...
  if (current.runtimeState.evidence_plan_revision !== evidencePlanRevision(readDraftDefinitionFromBody(current.body), current.runtimeState.claim_evidence ?? []))
    fail2("CLAIM_EVIDENCE_STALE", "canonical evidence plan revision does not match its definitions.");
}
function assertTestStrategyExecutionTransition(current, delta) {
...
function assertReviewExecutionEligible(current, execution) {
  if (execution.step_id !== current.runtimeState.active_step_id) {
    fail2("REVIEW_EXECUTION_STALE", "review-change requires an execution for the active step.");
  }
  ...
  if (laterCompletion) {
    fail2("REVIEW_EXECUTION_ALREADY_COMPLETED", "the recorded execution has already passed review and completed its step.");
  }
  const checkpoint = effectiveCheckpointPolicy(resolveCanonicalTaskStep(current));
  if (execution.mode === "default") {
    if (checkpoint !== "required") {
      fail2("REVIEW_CHECKPOINT_NOT_REQUIRED", "the current step does not admit a review checkpoint.");
    }
    if (execution.status !== "in-progress" || execution.advancement !== "not-applicable" || current.runtimeState.active_step_status !== "in-progress") {
      fail2("REVIEW_EXECUTION_NOT_REVIEWABLE", "the default execution is not awaiting its required review checkpoint.");
    }
    return;
  }
  if (execution.status !== "completed" || execution.advancement !== "repair-awaiting-verification" || current.runtimeState.active_step_status !== "completed") {
    fail2("REVIEW_EXECUTION_NOT_REVIEWABLE", "the repair execution is not awaiting verification review.");
  }
}
function validateTaskStateDelta(value) {
  ...
  if (action === "create-draft" || action === "update-draft") {
    ...
    if (record.claim_evidence !== undefined)
      result2.claim_evidence = validateClaimEvidence(record.claim_evidence, "semantic_delta.claim_evidence");
    return result2;
  }
...
function validateRuntimeProposal(value) {
  ...
  const writesTaskBasis = semanticDelta.kind === "task-state" && ["create-draft", "update-draft", "commit-replan"].includes(semanticDelta.action);
  const targetCount = writesTaskBasis || operationKind === "lifecycle-transaction" && mode !== "supersede" || operationKind === "archive-transaction" ? 2 : 1;
  ...
  if (writesTaskBasis) {
    const taskId = semanticDelta.kind === "task-state" && (semanticDelta.action === "create-draft" || semanticDelta.action === "update-draft") ? semanticDelta.task_id : sourceTuple.task_id;
    const expectedBasisPath = taskBasisRelativePath(sourceTuple.path, taskId);
    if (requestedTargets[1] !== expectedBasisPath) {
      fail2("RUNTIME_PATH_INVALID", `draft proposal second write target must be the identity-derived task basis ${expectedBasisPath}.`);
    }
  }
  if (operationKind === "task-state-transaction") {
    if (caller === "prepare-task") {
      if (mode === "default") {
        if (semanticDelta.kind !== "task-state" || !["clear-resume-review-gate", "create-draft", "update-draft", ...CLAIM_EVIDENCE_MIGRATION_ACTIONS].includes(semanticDelta.action)) {
          fail2("RUNTIME_CALLER_NOT_BOUND", "prepare-task default mode is bound only to clear-resume-review-gate, create-draft, update-draft, or migrate-claim-evidence.");
        }
        if (semanticDelta.kind === "task-state" && semanticDelta.action === "migrate-claim-evidence") {
          ...
          if (missingPreconditions.length > 0)
            fail2("RUNTIME_PRECONDITION_MISSING", `migrate-claim-evidence is missing required preconditions: ${missingPreconditions.join(", ")}.`);
        }
      ...
      if (mode !== "default")
        fail2("RUNTIME_MODE_INVALID", "review-change task-state proposals must use default mode.");
      if (semanticDelta.kind !== "task-state" || semanticDelta.action !== "record-review-result") {
        fail2("RUNTIME_CALLER_NOT_BOUND", "review-change is bound only to record-review-result.");
      }
      ...
      if (semanticDelta.kind !== "task-state" || !["step-progress", "record-step-preflight", "retry-step"].includes(semanticDelta.action))
  ...
  const claimRefs = semanticDelta.kind === "task-state" && "claim_evidence" in semanticDelta && semanticDelta.claim_evidence !== undefined ? claimEvidenceRefs(semanticDelta.claim_evidence) : [];
...
function validateDraftAuditLogEntry(value, location, taskId, taskSlug) {
  ...
  if (action === "create-draft" || action === "update-draft") {
    ...
    const claimEvidenceDigest = value.claim_evidence_digest === undefined ? undefined : expectString2(value.claim_evidence_digest, `${location}.claim_evidence_digest`);
    if (claimEvidenceDigest !== undefined && !/^[a-f0-9]{64}$/.test(claimEvidenceDigest))
      fail2("RUNTIME_SCHEMA_INVALID", `${location}.claim_evidence_digest must be SHA-256.`);
    return {
      action,
      idempotency_key: expectString2(value.idempotency_key, `${location}.idempotency_key`, SAFE_KEY_PATTERN2),
...
function validateClaimEvidenceMigrationAuditLogEntry(value, location, taskId, taskSlug) {
  ...
  if (value.action !== "migrate-claim-evidence" || value.operation_kind !== "task-state-transaction" || value.caller !== "prepare-task" || value.mode !== "default") {
    fail2("RUNTIME_STATE_CONFLICT", `${location} claim evidence migration audit has an invalid operation binding.`);
  }
  const fromTaskId = expectString2(value.from_task_id, `${location}.from_task_id`);
  const fromTaskSlug = expectString2(value.from_task_slug, `${location}.from_task_slug`);
  const entryTaskId = expectString2(value.task_id, `${location}.task_id`);
  ...
  if (fromTaskId !== entryTaskId || fromTaskSlug !== entryTaskSlug)
    fail2("RUNTIME_STATE_CONFLICT", `${location} migration must preserve task identity.`);
  const fromDocumentId = expectString2(value.from_document_id, `${location}.from_document_id`);
  const documentId = expectString2(value.document_id, `${location}.document_id`);
  if (!DOCUMENT_ID_PATTERN.test(fromDocumentId) || !DOCUMENT_ID_PATTERN.test(documentId)) {
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.document_id fields are invalid.`);
  ...
  if (fromDocumentId !== documentId)
    fail2("RUNTIME_STATE_CONFLICT", `${location} migration must preserve document_id.`);
  if (value.from_workflow_status !== "active" || value.from_lifecycle_state !== "active" || value.to_workflow_status !== "active" || value.to_lifecycle_state !== "active") {
    fail2("RUNTIME_STATE_CONFLICT", `${location} migration must preserve the active + active lifecycle tuple.`);
  }
  const sourceRevision = expectString2(value.source_revision, `${location}.source_revision`);
  if (!SHA256_PATTERN2.test(sourceRevision))
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.source_revision must be SHA-256.`);
  const authorityEvidence = validateAuthorityEvidence2(value.authority_evidence);
  const evidenceRefs = validateEvidenceRefs(value.evidence_refs, `${location}.evidence_refs`);
  const claimEvidenceDigest = expectString2(value.claim_evidence_digest, `${location}.claim_evidence_digest`);
  if (!SHA256_PATTERN2.test(claimEvidenceDigest))
    fail2("RUNTIME_SCHEMA_INVALID", `${location}.claim_evidence_digest must be SHA-256.`);
  return {
...
function validateVNextRuntimeState(value) {
  ...
  const optionalRuntimeStateFields = ["business_evidence_version", "evidence_plan_revision", "claim_evidence_required", "claim_evidence", "pending_review_result", "review_coverage", "step_attempts"];
  ...
  if (missingRuntimeStateFields.length > 0 || extraRuntimeStateFields.length > 0) {
    fail2("RUNTIME_SCHEMA_INVALID", `runtime_state keys mismatch; missing=[${missingRuntimeStateFields.join(", ")}], unexpected=[${extraRuntimeStateFields.join(", ")}].`);
  }
  if (runtime.business_evidence_version !== undefined && runtime.business_evidence_version !== 1) {
    fail2("TASK_SEMANTICS_VERSION_UNSUPPORTED", "runtime_state.business_evidence_version must be 1 when present.");
  }
  ...
  const claimEvidenceRequired = runtime.claim_evidence_required === undefined ? runtime.claim_evidence !== undefined : expectBoolean(runtime.claim_evidence_required, "runtime_state.claim_evidence_required");
  ...
  if (pendingReviewResult !== null) {
    if (pendingReviewResult.step_id !== activeStepId) {
      fail2("RUNTIME_STATE_CONFLICT", "runtime_state.pending_review_result must belong to the active step.");
    }
    if (pendingReviewResult.cycle_id !== reviewCycle.id) {
      fail2("RUNTIME_STATE_CONFLICT", "runtime_state.pending_review_result must belong to the current review cycle.");
    }
  }
  return {
    schema_version: 1,
...
var VNEXT_TASK_BASIS_KIND = "vnext-task-basis";
var TASK_BASIS_HEADING_ALIASES = ["任务输入依据", "Task Basis"];
function taskBasisRelativePath(currentTaskPath, taskId) {
  try {
    validateTaskId(taskId);
  } catch (error) {
    fail2("RUNTIME_IDENTITY_INVALID", error instanceof Error ? error.message : String(error));
  }
  const directory = path4.posix.dirname(normalizeRepoPath2(currentTaskPath, "CURRENT_TASK source path"));
  return path4.posix.join(directory, "task-basis", `TASK_BASIS-${taskId}.md`);
}
function taskBasisFilePath(root, relativePath) {
  const resolvedRoot = path4.resolve(root);
  const filePath = path4.resolve(resolvedRoot, ...normalizeRepoPath2(relativePath, "task basis path").split("/"));
  const relative2 = path4.relative(resolvedRoot, filePath).replace(/\\/g, "/");
...
function renderTaskBasisContent(identity, basis) {
  const frontmatter = {
    schema_version: 1,
...
`).trim();
  ...
  if (!match)
    fail2("TASK_BASIS_REFERENCE_INVALID", "CURRENT_TASK task basis reference must contain one exact path and SHA-256 revision.");
  return {
    path: normalizeRepoPath2(match[1], "CURRENT_TASK task basis path"),
    revision: match[2]
  };
}
function renderTaskBasisReference(reference) {
...
` + body.slice(existing.contentEnd);
  ...
  if (!background)
    fail2("RUNTIME_SECTION_INVALID", "CURRENT_TASK is missing the background section required to insert its task basis reference.");
  return body.slice(0, background.headingStart) + `## 任务输入依据

${rendered}

` + body.slice(background.headingStart);
}
function renderNewDraftBody(identity, definition, runtimeState, taskBasisReference) {
...
function readCanonicalTaskBasis(root, current = readCanonicalCurrentTask(root)) {
  ...
  if (!reference)
    fail2("TASK_BASIS_MISSING", "CURRENT_TASK does not link an exact task basis.");
  const expectedPath = taskBasisRelativePath(current.relativePath, current.runtimeState.task_id);
  if (reference.path !== expectedPath) {
    fail2("TASK_BASIS_REFERENCE_INVALID", `CURRENT_TASK task basis path must be ${expectedPath}.`);
  }
  const filePath = taskBasisFilePath(root, reference.path);
  if (!fs3.existsSync(filePath))
    fail2("TASK_BASIS_MISSING", `Linked task basis is missing: ${reference.path}`);
  const content = fs3.readFileSync(filePath, "utf8");
  const revision = sha2562(content);
  if (revision !== reference.revision)
    fail2("TASK_BASIS_REVISION_CONFLICT", "Linked task basis revision does not match its file content.");
  const { frontmatter, body } = parseYamlFrontmatter(content, reference.path);
  expectExactKeys2(frontmatter, ["schema_version", "kind", "task_id", "document_id", "original_request", "user_decisions"], `${reference.path} frontmatter`);
  if (frontmatter.schema_version !== 1 || frontmatter.kind !== VNEXT_TASK_BASIS_KIND) {
    fail2("TASK_BASIS_INVALID", `${reference.path} is not a supported vNext task basis.`);
  }
  const taskId = expectString2(frontmatter.task_id, `${reference.path}.task_id`);
  const documentId = expectString2(frontmatter.document_id, `${reference.path}.document_id`);
  if (taskId !== current.runtimeState.task_id || documentId !== current.sourceTuple.document_id) {
    fail2("TASK_BASIS_IDENTITY_CONFLICT", "Linked task basis identity does not match CURRENT_TASK.");
  }
  if (!/^# vNext TASK_BASIS\s*$/mu.test(body))
    fail2("TASK_BASIS_INVALID", `${reference.path} is missing its canonical heading.`);
...
function closureEligibilityBlockers(root, current, delta, archiveAlreadyExists) {
  ...
  try {
    ...
    if (repairRecords.length > 0) {
      ...
      const verified = current.runtimeState.execution_log.some((item) => {
        ...
        return receipt !== undefined && receipt.admitted_fingerprints.length === repairFingerprints.length && repairFingerprints.every((fingerprint) => receipt.admitted_fingerprints.includes(fingerprint)) && (repairTargets.length === 0 || receipt.change_set_id === repairTargets[0]);
      });
      if (!verified)
        blockers.push("every repair route must have a durable same-diff verification receipt before closure.");
      if (repairRecords.some((item) => !(item.repair_fingerprint || item.repair_fingerprints?.length) || !item.change_set_id))
        blockers.push("a repair execution record is missing its finding fingerprints or logical change-set identity.");
    }
  } catch (error) {
    blockers.push(error instanceof Error ? error.message : String(error));
  }
  ...
  if (!claimEvidenceStateEnabled(current.runtimeState)) {
    ...
    if (!durableClaimEvidence.acceptance_satisfied)
      blockers.push("durable claim-bound acceptance evidence is incomplete.");
    if (!durableClaimEvidence.validation_complete)
      blockers.push("durable claim-bound validation evidence is incomplete.");
  }
...
function prepareArchiveTransaction(root, current, proposal, now) {
  ...
  const blockers = closureEligibilityBlockers(root, current, delta, fs3.existsSync(archiveTarget.filePath));
  if (blockers.length > 0)
...
function replaceStatusSectionBody(content, sectionKey, lines, location) {
  const section = requiredStatusSection(content, sectionKey, location);
...
function makeClaimEvidenceMigrationAudit(current, proposal, next, now) {
  if (proposal.semantic_delta.kind !== "task-state" || proposal.semantic_delta.action !== "migrate-claim-evidence") {
    fail2("RUNTIME_SCHEMA_INVALID", "Only claim evidence migration transitions may create a claim evidence migration audit record.");
  }
  if (next.workflow_status !== "active" || next.lifecycle_state !== "active") {
    fail2("RUNTIME_STATE_CONFLICT", "Claim evidence migration must preserve the active + active lifecycle tuple.");
  }
  const delta = proposal.semantic_delta;
  return {
    action: "migrate-claim-evidence",
  ...
  if (delta.action === "create-draft" || delta.action === "update-draft") {
    ...
    if (draftDelta.claim_evidence !== undefined) {
      if (audit.claim_evidence_digest !== digest(draftDelta.claim_evidence)) {
        fail2("RUNTIME_REPLAY_INCOMPLETE", `${delta.action} replay claim evidence digest does not match the proposal.`);
      }
      if (digest(current.runtimeState.claim_evidence ?? []) !== digest(draftDelta.claim_evidence)) {
        fail2("RUNTIME_REPLAY_INCOMPLETE", `${delta.action} replay no longer has the proposal claim evidence in canonical CURRENT_TASK.`);
      }
    }
    const basis = readCanonicalTaskBasis(root, current);
    if (digest(basis.basis) !== digest(draftDelta.task_basis)) {
      fail2("RUNTIME_REPLAY_INCOMPLETE", `${delta.action} replay no longer has the proposal task basis.`);
    }
...
function expectedClaimEvidenceMigrationReplayAudit(current, proposal) {
  const entry = current.runtimeState.execution_log.find((item) => ("action" in item) && item.action === "migrate-claim-evidence" && item.idempotency_key === proposal.idempotency_key);
  if (!entry)
...
function assertClaimEvidenceMigrationReplay(current, proposal) {
  ...
  if (current.runtimeState.claim_evidence_required !== true || current.runtimeState.claim_evidence.length === 0) {
    fail2("RUNTIME_REPLAY_INCOMPLETE", "claim evidence migration replay no longer has a strict non-empty plan.");
  }
  ...
  if (digest(current.runtimeState.claim_evidence) !== digest(delta.claim_evidence)) {
    fail2("RUNTIME_REPLAY_INCOMPLETE", "claim evidence migration replay no longer has the proposal plan in canonical CURRENT_TASK.");
  }
  if (audit.idempotency_key !== proposal.idempotency_key || audit.source_revision !== proposal.source_tuple.revision || audit.evidence_refs.join("|") !== delta.evidence_refs.join("|") || audit.claim_evidence_digest !== digest(delta.claim_evidence) || audit.task_id !== current.runtimeState.task_id || audit.task_slug !== current.runtimeState.task_slug || audit.document_id !== current.sourceTuple.document_id) {
    fail2("RUNTIME_REPLAY_INCOMPLETE", "claim evidence migration replay audit does not match the proposal identity, plan, or evidence.");
  }
}
...
function assertStepProgressReplay(current, proposal) {
  ...
  if (entry.mode === "repair" && entry.repair_fingerprint === undefined && entry.repair_fingerprints === undefined) {
    fail2("RUNTIME_REPLAY_INCOMPLETE", "repair replay is missing its durable finding fingerprints.");
  }
...
function assertReviewResultReplay(current, proposal) {
  ...
  if (!pending || pending.review_id !== proposal.semantic_delta.review_result.review_id) {
    fail2("RUNTIME_REPLAY_INCOMPLETE", "review-result replay is no longer the pending result for the current execution generation.");
  }
  const { recorded_at: _recordedAt, ...stored } = pending;
  if (digest(stored) !== digest(proposal.semantic_delta.review_result)) {
    fail2("RUNTIME_REPLAY_INCOMPLETE", "review-result replay does not match the durable pending result.");
  }
}
function assertTaskStateReplay(root, current, proposal) {
  ...
  if (delta.action === "mark-replan-blocked") {
    ...
    if (digest(basis.basis) !== digest(commitDelta.task_basis)) {
      fail2("RUNTIME_REPLAY_INCOMPLETE", "commit-replan replay no longer has the proposal task basis.");
    }
    if (commitDelta.claim_evidence !== undefined && digest(current.runtimeState.claim_evidence ?? []) !== digest(commitDelta.claim_evidence)) {
      fail2("RUNTIME_REPLAY_INCOMPLETE", "commit-replan replay no longer has the proposal claim evidence in canonical CURRENT_TASK.");
    }
  }
...
function applyTaskStateDelta(root, current, proposal, now) {
  ...
  if (delta.action === "migrate-claim-evidence") {
    ...
    if (current.runtimeState.workflow_status !== "active" || current.runtimeState.lifecycle_state !== "active") {
      fail2("CLAIM_EVIDENCE_MIGRATION_NOT_APPLICABLE", "claim evidence migration requires a legacy active + active task.");
    }
    if (current.runtimeState.claim_evidence.length > 0) {
      fail2("CLAIM_EVIDENCE_MIGRATION_NOT_APPLICABLE", "claim evidence migration cannot replace an existing structured claim_evidence plan.");
    }
  ...
  if (delta.action === "update-draft") {
    ...
    for (const claim of claimEvidence) {
      ...
      if (previous && (previous.requirement !== claim.requirement || previous.source_ref !== claim.source_ref || previous.claim_kind !== claim.claim_kind))
        fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", "Changing a draft obligation requires a new claim identity.");
      for (const slot of claim.slots) {
        const old = previous?.slots.find((item) => item.slot_id === slot.slot_id);
        if (old && digest(evidenceSlotDefinition(old)) !== digest(evidenceSlotDefinition(slot)))
          fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", "Changing a draft check requires a new slot/check identity.");
        ...
        if (previousCheck && digest(previousCheck) !== digest(slot.check))
          fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", "A check_id cannot be reassigned to a different check during draft refinement.");
      }
    }
    const nextWithoutAudit = {
      ...current.runtimeState,
  ...
  if (delta.action === "record-review-result") {
    ...
    if (current.runtimeState.workflow_status !== "active" || current.runtimeState.lifecycle_state !== "active") {
      fail2("TASK_STATE_NOT_ACTIVE", "review results may be recorded only for an active + active task.");
    }
    if (current.runtimeState.resume_requires_review) {
      fail2("RESUME_REVIEW_REQUIRED", "review-change cannot record a step review while the resume review gate is active.");
    }
    ...
    if (current.runtimeState.review_coverage && !review.test_assessment)
      fail2("REVIEW_ASSESSMENT_REQUIRED", "Cumulative reviews require test necessity, oracle, boundary, reuse and applicability assessment.");
    if (current.runtimeState.review_coverage && review.test_assessment && !review.test_assessment.applicable && (parseMutationScope(current.body).persistent_tests?.length || current.runtimeState.claim_evidence?.some((claim) => claim.slots.some((slot) => slot.check?.method === "execution"))))
      fail2("REVIEW_ASSESSMENT_REQUIRED", "Declared execution checks or persistent tests require an applicable assessment even when test files are unchanged.");
    ...
    if (!executionResult || execution.change_set_id !== review.change_set_id || executionResult.change_set_id !== review.change_set_id || executionResult.review_target.revision !== review.review_target_revision) {
      fail2("REVIEW_TARGET_CONFLICT", "review result does not bind the Runtime-recorded execution change set and review target.");
    }
    ...
    if (currentTarget.revision !== review.review_target_revision) {
      fail2("REVIEW_TARGET_STALE", "product files changed after the reviewed execution target was recorded.");
    }
    const expectedPhase = execution.mode === "repair" ? "verification" : "discovery";
    if (review.cycle_phase !== expectedPhase)
      fail2("REVIEW_PHASE_INVALID", `review result must use ${expectedPhase} for the latest execution.`);
    if (review.cycle_phase === "discovery" && review.unresolved_fingerprints.length > 0) {
      fail2("REVIEW_PHASE_INVALID", "discovery review cannot reference unresolved admitted findings.");
    }
    const openFingerprints = new Set(current.runtimeState.findings.filter((item) => item.review_cycle_id === review.cycle_id && ["admitted", "in-progress"].includes(item.status)).map((item) => item.fingerprint));
    for (const fingerprint of review.unresolved_fingerprints) {
      if (!openFingerprints.has(fingerprint))
        fail2("FINDING_NOT_FOUND", `review result references non-open finding ${fingerprint}.`);
    }
    ...
    if (![...nestedEvidence, ...review.evidence_refs].every((ref) => delta.evidence_refs.includes(ref))) {
      fail2("RUNTIME_EVIDENCE_INVALID", "record-review-result evidence_refs must cover the review result and every finding.");
    }
    return {
      next: {
        ...current.runtimeState,
  ...
  if (delta.action === "commit-replan") {
    ...
    const planRevision = assertEvidencePlan(delta.replacement_definition, claimEvidence, true);
    ...
    if (current.runtimeState.evidence_plan_revision === planRevision && current.runtimeState.step_attempts?.[current.runtimeState.active_step_id]?.attempts.at(-1)?.status === "blocked")
      fail2("RETRY_ATTEMPT_CONFLICT", "Same-plan replan cannot reset a blocked attempt; use bounded retry or diagnose the failure.");
    const nextWithoutAudit = {
      ...current.runtimeState,
      business_evidence_version: 1,
      step_attempts: current.runtimeState.evidence_plan_revision === planRevision ? current.runtimeState.step_attempts ?? {} : {},
      workflow_status: "active",
      lifecycle_state: "active",
      resume_requires_review: false,
      resume_review_reasons: [],
      ...
      claim_evidence: copyClaimEvidence(claimEvidence),
      evidence_plan_revision: planRevision,
      review_coverage: current.runtimeState.review_coverage ? { ...current.runtimeState.review_coverage, last_clean_revision: null, pending_paths: current.runtimeState.review_coverage.base.entries.map((entry) => entry.path) } : emptyReviewCoverage(planRevision),
      applied_proposals: appendAppliedProposal(current.runtimeState, proposal, current.sourceTuple.revision)
    };
    const audit = makeReplanAudit(current, proposal, nextWithoutAudit, now);
    return {
  ...
  if (delta.action === "retry-step") {
    ...
    if (proposal.mode !== "default" || current.runtimeState.workflow_status !== "active" || current.runtimeState.lifecycle_state !== "active" || current.runtimeState.resume_requires_review || delta.step_id !== current.runtimeState.active_step_id || current.runtimeState.active_step_status !== "blocked")
      fail2("RETRY_STATE_INVALID", "Retry requires the active blocked ordinary step.");
    if (current.runtimeState.pending_review_result || current.runtimeState.findings.some((f) => ["admitted", "in-progress"].includes(f.status)))
      fail2("RETRY_FINDINGS_BLOCKED", "Retry cannot consume pending review or findings.");
  ...
  if (!currentClaimEvidenceEnabled && delta.claim_evidence !== undefined) {
    fail2("CLAIM_EVIDENCE_MIGRATION_REQUIRED", "execute-step cannot create a claim_evidence plan for a legacy task; prepare-task refinement/migration must persist the plan first.");
  }
  if (currentClaimEvidenceEnabled) {
    ...
    if (plannedClaimEvidence.length === 0) {
      fail2("CLAIM_EVIDENCE_REQUIRED", "the strict task has an empty claim_evidence plan; prepare-task must repair it before execution.");
    }
  ...
  if (executionMode === "repair") {
    if (deltaRepairFingerprints.length === 0)
      fail2("FINDING_ADMISSION_REQUIRED", "repair mode requires one or more admitted finding fingerprints.");
    for (const fingerprint of deltaRepairFingerprints) {
      ...
      if (!finding || !["admitted", "in-progress"].includes(finding.status))
        fail2("FINDING_ADMISSION_REQUIRED", `repair fingerprint ${fingerprint} is not an admitted current-task finding.`);
      if (delta.repair_wave_id && finding.last_repair_wave_id !== delta.repair_wave_id) {
        fail2("REPAIR_WAVE_CONFLICT", `repair fingerprint ${fingerprint} was not recorded in repair wave ${delta.repair_wave_id}.`);
      }
    }
    if (!delta.change_set_id)
      fail2("REPAIR_CHANGE_SET_REQUIRED", "repair mode requires one Runtime-owned logical change_set_id.");
    ...
    if (deltaRepairFingerprints.length > 0 || delta.repair_wave_id !== undefined)
      fail2("RUNTIME_MODE_INVALID", "default execution cannot carry repair bookkeeping.");
    ...
    if (delta.change_set_id !== undefined && delta.review_receipt !== undefined && delta.change_set_id !== delta.review_receipt.change_set_id) {
      fail2("REVIEW_TARGET_CONFLICT", "step-progress change_set_id must match the review receipt change_set_id.");
    }
  }
  ...
  if (delta.review_receipt !== undefined && delta.execution_result !== undefined) {
    fail2("REVIEWED_COMPLETION_EXECUTION_RESULT_FORBIDDEN", "reviewed step completion cannot record a new execution result or acceptance evidence.");
  }
  if (delta.execution_result !== undefined) {
    if (!executionChangeSetId || delta.execution_result.change_set_id !== executionChangeSetId) {
      fail2("REVIEW_TARGET_CONFLICT", "execution_result must bind the step-progress Runtime change_set_id.");
    }
    ...
    if (currentTarget.revision !== delta.execution_result.review_target.revision) {
      fail2("REVIEW_TARGET_STALE", "product files changed while the execution result was being recorded.");
    }
    if (delta.execution_result.outcome === "blocked" !== (newStatus === "blocked")) {
      fail2("RUNTIME_STATE_CONFLICT", "execution_result outcome must match the step-progress status.");
    }
    ...
    if (structuredEvidenceRefs.length !== delta.evidence_refs.length || structuredEvidenceRefs.some((item) => !delta.evidence_refs.includes(item))) {
      fail2("RUNTIME_STATE_CONFLICT", "step-progress evidence_refs must exactly match the structured execution_result evidence.");
    }
  }
  ...
  if (currentClaimEvidenceEnabled && delta.claim_evidence !== undefined) {
    assertClaimEvidencePlanPreserved(current.runtimeState.claim_evidence ?? [], delta.claim_evidence, "semantic_delta.claim_evidence");
    if (delta.review_receipt !== undefined && digest(current.runtimeState.claim_evidence ?? []) !== digest(delta.claim_evidence)) {
  ...
  for (const record of transitionClaimEvidence)
    for (const slot of record.slots) {
      ...
      if (slot.report && digest(old) !== digest(slot))
        assertEvidenceReportApplicable(root, current, slot);
      if (old?.report && slot.report?.result_id === old.report.result_id && (digest(old.report) !== digest(slot.report) || digest(old.evidence_refs) !== digest(slot.evidence_refs)))
        fail2("CLAIM_EVIDENCE_PLAN_CONFLICT", "A changed report or artifact mapping requires a new result_id.");
      if (slot.before_step_id === delta.step_id && !slot.prerequisite_receipt)
        fail2("PREREQUISITE_REQUIRED", "record-step-preflight must consume prerequisites before any execution result or progress.");
    }
  ...
  for (const result of delta.execution_result?.acceptance_evidence ?? []) {
    ...
    if (!slot || slot.check?.check_id !== result.check_id || slot.minimum_type !== result.minimum_type || slot.disposition !== result.disposition || digest(slot.report) !== digest(result.report) || digest(slot.evidence_refs) !== digest(result.evidence_refs))
  ...
  if (newStatus === "completed" && !evaluateClaimEvidence(transitionClaimEvidence, { root, current, due_step_id: delta.step_id }).validation_complete)
    fail2("CLAIM_EVIDENCE_INCOMPLETE", "Every due slot must have applicable successful evidence before step completion.");
  let advancement = {
  ...
  if (executionMode === "repair" && newStatus === "completed") {
    ...
    if (current.runtimeState.review_coverage && checkpoint === "not-required" && stepResolution.next && !delta.execution_result)
      fail2("REVIEW_EXECUTION_REQUIRED", "An exempt intermediate step must record its actual execution before advancement.");
    ...
    if (delta.review_receipt !== undefined) {
      ...
      if (!pendingReview || pendingReview.verdict !== "clean") {
        fail2("CLEAN_REVIEW_REQUIRED", "step completion requires the canonical pending clean review result.");
      }
      if (pendingReview.step_id !== delta.step_id || pendingReview.cycle_id !== delta.review_receipt.cycle_id || pendingReview.cycle_phase !== delta.review_receipt.cycle_phase || pendingReview.change_set_id !== delta.review_receipt.change_set_id || pendingReview.review_target_revision !== delta.review_receipt.review_target_revision || digest(pendingReview.evidence_refs) !== digest(delta.review_receipt.evidence_refs)) {
        fail2("REVIEW_RECEIPT_CONFLICT", "step completion review receipt must match the canonical pending clean review.");
      }
      const reviewedExecution = current.runtimeState.execution_log.map((item) => ("action" in item) ? item : cumulativeReviewExecution(current, item)).find((item) => !("action" in item) && item.idempotency_key === pendingReview.execution_id);
      if (!reviewedExecution?.execution_result || reviewedExecution.change_set_id !== pendingReview.change_set_id || reviewedExecution.execution_result.review_target.revision !== pendingReview.review_target_revision) {
        fail2("REVIEW_TARGET_CONFLICT", "canonical clean review no longer binds its recorded execution target.");
      }
      ...
      if (currentTarget.revision !== pendingReview.review_target_revision) {
        fail2("REVIEW_TARGET_STALE", "product files changed after the clean review was recorded.");
      }
    ...
    if (currentStepRepairLogs.length > 0) {
      const repairFingerprints = [...new Set(currentStepRepairLogs.flatMap((item) => {
        if (!item.change_set_id)
          fail2("REPAIR_CHANGE_SET_REQUIRED", "a repair execution record is missing its logical change-set identity.");
        const fingerprints = item.repair_fingerprints ?? (item.repair_fingerprint ? [item.repair_fingerprint] : []);
        if (fingerprints.length === 0)
          fail2("REPAIR_VERIFICATION_REQUIRED", "a repair execution record is missing its finding fingerprints.");
        return fingerprints;
      }))];
      const repairTargets = [...new Set(currentStepRepairLogs.map((item) => item.change_set_id))];
      if (repairTargets.length !== 1)
        fail2("REPAIR_CHANGE_SET_CONFLICT", "all repair attempts for one step must use the same logical change set.");
      ...
      if (receipt.change_set_id !== repairTargets[0])
        fail2("REPAIR_CHANGE_SET_CONFLICT", "verification must cover the exact logical change set repaired by the current step.");
    ...
    if (stepResolution.next) {
      ...
      if (!claimEvidenceRequired) {
        fail2("CLAIM_EVIDENCE_MIGRATION_REQUIRED", "legacy CURRENT_TASK requires prepare-task refinement/migration before task-complete; aggregate evidence cannot provide terminal completion.");
      }
      if (delta.claim_evidence === undefined) {
        fail2("CLAIM_EVIDENCE_REQUIRED", "the final task-complete step-progress must carry the durable claim_evidence snapshot.");
      }
      const completion = evaluateClaimEvidence(transitionClaimEvidence, { root, current });
      if (!completion.validation_complete) {
        fail2("CLAIM_EVIDENCE_INCOMPLETE", "task-complete requires every planned claim evidence slot to be existing, reused, or newly-executed with evidence_refs.");
      }
      if (!completion.acceptance_satisfied) {
        fail2("CLAIM_EVIDENCE_ACCEPTANCE_REQUIRED", "task-complete requires at least one complete acceptance claim; invariant evidence cannot substitute for acceptance evidence.");
      }
      advancement = {
        ...advancement,
  ...
  if (executionMode === "default" && delta.execution_result) {
    ...
    if (!ledger && result.attempt_id && result.attempt_id !== nextStepAttemptId(current))
      fail2("RETRY_ATTEMPT_CONFLICT", "Initial attempt identity is Runtime-derived.");
    if (ledger && (ledger.evidence_plan_revision !== current.runtimeState.evidence_plan_revision || result.attempt_id !== activeAttempt?.attempt_id || activeAttempt.status === "ready" || activeAttempt.status === "blocked"))
      fail2("RETRY_PREFLIGHT_REQUIRED", "Results must bind the current preflighted attempt; old receipts cannot run or complete a retry.");
    ...
    if (result.blocker_kind === "environment" && (result.outcome !== "blocked" || observations.some((r) => r.status === "failed") || !observations.some((r) => r.status === "blocked"))) {
      fail2("RETRY_DIAGNOSIS_REQUIRED", "Environment classification requires a blocked execution without business failures.");
    }
  ...
  if (executionMode === "default" && newStatus === "completed" && !delta.execution_result && stepAttempts?.[delta.step_id]?.attempts.at(-1)?.status !== undefined && stepAttempts[delta.step_id].attempts.at(-1).status !== "implemented")
    fail2("RETRY_EXECUTION_REQUIRED", "A recovered attempt must run and report before completion.");
  ...
  if (delta.execution_result && coverage) {
    ...
    if (result.change_set_id !== coverage.change_set_id)
      fail2("REVIEW_TARGET_CONFLICT", "Execution must retain the cumulative change set.");
    for (const entry of result.review_base.entries) {
      if (digest(entry) !== digest(coverage.target.entries.find((e) => e.path === entry.path)))
        fail2("REVIEW_PREFLIGHT_REQUIRED", "Execution base must match the registered before-state.");
    }
    ...
    if (digest([...changed].sort()) !== digest([...result.actual_changed_paths].sort()))
      fail2("REVIEW_TARGET_CONFLICT", "Execution must account for all changes in the cumulative target.");
  ...
  if (coverage && advancement.outcome === "task-complete" && checkpoint === "not-required") {
    if (!(stepResolution.current.checkpoint_boundary?.startsWith("final-exemption:") && stepResolution.current.checkpoint_boundary.slice("final-exemption:".length).trim()))
      fail2("REVIEW_CHECKPOINT_REQUIRED", "Final exemption must explicitly cover the cumulative task.");
...
function rollbackDraftTransactionAndVerify(root, current, artifact, originalTaskBasisContent, readCurrentTask) {
  try {
    ...
    if (originalTaskBasisContent === undefined && fs3.existsSync(artifact.filePath)) {
      fs3.rmSync(artifact.filePath, { force: true });
    }
  } catch (error) {
  ...
  try {
    ...
    if (originalTaskBasisContent === undefined) {
      if (fs3.existsSync(artifact.filePath))
        return { verified: false, detail: "rollback left a newly-created task basis behind." };
    } else if (!fs3.existsSync(artifact.filePath) || fs3.readFileSync(artifact.filePath, "utf8") !== originalTaskBasisContent) {
...
class GovernanceTransactionKernel {
  ...
  apply(rawProposal, options = {}) {
    ...
    try {
      ...
      if (proposal.operation_kind === "lifecycle-transaction") {
      ...
      } else if (proposal.operation_kind === "task-state-transaction" && proposal.semantic_delta.kind === "task-state" && ["create-draft", "update-draft", "commit-replan"].includes(proposal.semantic_delta.action)) {
        ...
        if (proposal.requested_write_targets.length !== 2 || proposal.requested_write_targets[0] !== current.relativePath || proposal.requested_write_targets[1] !== expectedBasisPath) {
          fail2("RUNTIME_PATH_INVALID", "draft proposal must target the exact canonical CURRENT_TASK and identity-derived task basis paths.");
        }
    ...
    let taskBasisArtifact;
    let originalTaskBasisContent;
    try {
      if (transition.taskBasis) {
        ...
        const basisExists = fs3.existsSync(taskBasisArtifact.filePath);
        if (proposal.semantic_delta.kind === "task-state" && proposal.semantic_delta.action === "create-draft") {
          if (basisExists)
            fail2("TASK_BASIS_CONFLICT", `create-draft refuses to overwrite existing task basis ${taskBasisArtifact.path}.`);
        } else if (existingReference) {
          if (existingReference.path !== taskBasisArtifact.path) {
            fail2("TASK_BASIS_REFERENCE_INVALID", "CURRENT_TASK links a different task basis path than the draft transaction target.");
          }
          if (!basisExists)
            fail2("TASK_BASIS_MISSING", `Linked task basis is missing: ${taskBasisArtifact.path}`);
          originalTaskBasisContent = fs3.readFileSync(taskBasisArtifact.filePath, "utf8");
          if (sha2562(originalTaskBasisContent) !== existingReference.revision) {
            fail2("TASK_BASIS_REVISION_CONFLICT", "Linked task basis changed outside the Runtime transaction.");
          }
        } else if (basisExists) {
          fail2("TASK_BASIS_CONFLICT", `Unlinked task basis already exists at ${taskBasisArtifact.path}.`);
        }
      }
    } catch (error) {
      return buildResult("blocked", proposal, current, options, error instanceof Error ? error.message : String(error), {
    ...
    try {
      ...
      if (taskBasisArtifact) {
        ...
        if (basisReadBack.content !== taskBasisArtifact.content || basisReadBack.revision !== taskBasisArtifact.revision) {
          ...
          return buildResult("blocked", proposal, current, options, rollback.verified ? "Runtime task basis read-back did not match the staged artifact; rollback read-back verified." : `Runtime task basis read-back did not match the staged artifact; ${rollback.detail}`, { code: rollback.verified ? "READ_BACK_MISMATCH" : "ROLLBACK_FAILED" });
        }
      }
...
function assertOrdinaryPreflight(current, root) {
  ...
  if (pending) {
    ...
    if (!execution || "action" in execution || !execution.execution_result || captureReviewTarget(root, execution.execution_result.review_target.entries.map((entry) => entry.path)).revision !== pending.review_target_revision)
      fail2("REVIEW_TARGET_STALE", "Pending review target changed; preserve the review and resolve its conflict.");
    let route = pending.blocker?.next_route ?? "its recorded blocker route";
    if (pending.verdict === "clean")
      route = "complete-reviewed-step";
    if (pending.verdict === "findings")
      route = "begin-repair";
    fail2("PENDING_REVIEW_REQUIRED", `Pending review must be consumed by ${route ?? "its recorded blocker route"}; no ordinary execution is admitted.`);
  }
  if (current.runtimeState.findings.some((item) => ["admitted", "in-progress"].includes(item.status))) {
    fail2("REVIEW_CONVERGENCE_REQUIRED", "Open findings require admitted repair; ordinary execution cannot consume them.");
  }
  if (current.runtimeState.workflow_status !== "active" || current.runtimeState.lifecycle_state !== "active" || current.runtimeState.resume_requires_review || current.runtimeState.active_step_status === "blocked") {
    fail2("PREFLIGHT_BLOCKED", "ordinary execution requires an active unblocked task without a resume gate.");
  }
}
function createStepRetryProposal(current, input) {
  return validateRuntimeProposal({
    schema_version: 1,
    kind: VNEXT_RUNTIME_PROPOSAL_KIND,
    operation_kind: "task-state-transaction",
...
function createReviewResultProposal(current, input) {
...
function createPrepareTaskReplanProposal(current, input) {
  ...
  return validateRuntimeProposal({
    ...
    evidence_refs: proposalEvidenceRefs,
    idempotency_key: input.idempotency_key,
    requested_write_targets: input.delta.action === "commit-replan" ? [current.relativePath, taskBasisRelativePath(current.relativePath, current.runtimeState.task_id)] : [current.relativePath]
  });
}
function createFindingQueueProposal(current, input) {
  return validateRuntimeProposal({
    schema_version: 1,
    kind: VNEXT_RUNTIME_PROPOSAL_KIND,
...
import { parse as parse3, parseDocument as parseDocument3, stringify as stringify2 } from "yaml";
...
var SAFE_ID_PATTERN = /^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/u;
var SHA256_PATTERN3 = /^[a-f0-9]{64}$/u;
var TARGET_IDENTITY_PATTERN2 = /^[a-f0-9]{32}$/u;
var STABLE_ID_PATTERN = /^artifact-[a-f0-9]{24}$/u;
var CANONICAL_DOCUMENT_ID_PATTERN = /^doc-[a-f0-9]{24}$/u;
var CANONICAL_SCHEMA_VERSION = 1;
var CANONICAL_DOCUMENT_KIND = "vnext-canonical-document";
var CANONICAL_CONVERSION_RULE = "canonical-envelope-v1";
var CURRENT_TASK_FILE = "CURRENT_TASK.md";

class MigrationProvenanceError extends Error {
  code;
  constructor(message) {
...
function expectRecord3(value, location) {
  if (!isRecord3(value))
    fail3(`${location} must be a mapping.`);
  return value;
}
function expectString3(value, location) {
  if (typeof value !== "string" || value.trim().length === 0)
    fail3(`${location} must be a non-empty string.`);
  return value.trim();
}
function expectStringArray3(value, location, allowEmpty = false) {
  if (!Array.isArray(value) || !allowEmpty && value.length === 0)
    fail3(`${location} must be ${allowEmpty ? "an array" : "a non-empty array"}.`);
  const values = value.map((item, index) => expectString3(item, `${location}[${index}]`));
  if (new Set(values).size !== values.length)
    fail3(`${location} must not contain duplicates.`);
  return values;
}
function expectExactKeys3(value, expected, location) {
  const expectedSet = new Set(expected);
  const missing = expected.filter((key) => !(key in value));
  const extra = Object.keys(value).filter((key) => !expectedSet.has(key));
  if (missing.length > 0 || extra.length > 0)
...
function normalizeRoot(root) {
  ...
  const parsedRoot = path6.parse(resolved).root.replace(/\\/gu, "/");
...
function normalizeRepoPath4(value, location) {
  ...
  if (!normalized || normalized.startsWith("/") || /^[A-Za-z]:\//u.test(normalized) || normalized.split("/").some((segment) => segment === ".." || segment.length === 0) || /[\0-\x1F\x7F]/u.test(normalized)) {
    fail3(`${location} is not a safe repository-relative path: ${value}`);
  }
  return normalized;
}
function resolveRepoPath2(root, relative2, location) {
  const normalized = normalizeRepoPath4(relative2, location);
  const resolvedRoot = path6.resolve(root);
  const resolved = path6.resolve(resolvedRoot, ...normalized.split("/"));
  const prefix = resolvedRoot.endsWith(path6.sep) ? resolvedRoot : `${resolvedRoot}${path6.sep}`;
  if (resolved !== resolvedRoot && !resolved.startsWith(prefix))
    fail3(`${location} escapes the target root: ${relative2}`);
  return resolved;
}
function readJson(filePath, location) {
  try {
    return expectRecord3(JSON.parse(fs5.readFileSync(filePath, "utf8")), location);
  } catch (error) {
    if (error instanceof MigrationProvenanceError)
      throw error;
    fail3(`${location} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
}
function readYaml(filePath, location) {
  try {
    const document = parseDocument2(fs5.readFileSync(filePath, "utf8"), { uniqueKeys: true });
    const diagnostics = [...document.errors, ...document.warnings];
    if (diagnostics.length > 0)
      fail3(`${location} is invalid YAML: ${diagnostics.map((item) => item.message).join("; ")}`);
    return expectRecord3(document.toJS(), location);
  } catch (error) {
    if (error instanceof MigrationProvenanceError)
      throw error;
    fail3(`${location} is invalid YAML: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function sameRuntime(left, right) {
  if (left === null || right === null)
    return left === right;
  return left.kind === right.kind && left.package_path === right.package_path && left.entrypoint === right.entrypoint && left.package_version === right.package_version && left.node_min_version === right.node_min_version && left.package_lock_sha256 === right.package_lock_sha256 && left.entrypoint_sha256 === right.entrypoint_sha256;
}
function validateInstallState(value) {
  ...
  if (value.schema_version !== 1 || value.kind !== "vnext-install-state" || (legacyShape ? value.mode !== "pure-vnext" : value.distribution_state !== "vnext") || value.legacy_compatibility !== "absent" || value.recovery_boundary !== "in-progress-marker")
    fail3("INSTALL_STATE.json is not a valid vNext install state.");
  ...
  if (!/^migration-[a-f0-9]{24}$/u.test(migrationPackId) || !/^bundle-[a-f0-9]{24}$/u.test(bundleId))
    fail3("INSTALL_STATE.json has invalid migration or bundle identity.");
  ...
  if (!SHA256_PATTERN3.test(sourceTreeHash) || !TARGET_IDENTITY_PATTERN2.test(targetIdentity))
    fail3("INSTALL_STATE.json has invalid source/target identity fields.");
  ...
  if (!Array.isArray(value.managed_files) || value.managed_files.length === 0)
    fail3("INSTALL_STATE.json.managed_files must be non-empty.");
  const seen = new Set;
  const managedFiles = value.managed_files.map((raw, index) => {
    ...
    const relative2 = normalizeRepoPath4(expectString3(item.path, `INSTALL_STATE.json.managed_files[${index}].path`), `INSTALL_STATE.json.managed_files[${index}].path`);
    if (seen.has(relative2))
      fail3(`INSTALL_STATE.json.managed_files contains duplicate path ${relative2}.`);
    seen.add(relative2);
    const checksum = item.checksum === "" ? "" : expectString3(item.checksum, `INSTALL_STATE.json.managed_files[${index}].checksum`);
    if (!SHA256_PATTERN3.test(checksum) && !(relative2 === COMPLETED_MIGRATION_INSTALL_STATE_RELATIVE_PATH && checksum === ""))
      fail3(`INSTALL_STATE.json.managed_files[${index}].checksum is invalid.`);
    return { path: relative2, checksum, category: expectString3(item.category, `INSTALL_STATE.json.managed_files[${index}].category`) };
  });
...
function validateReceipt(value) {
  ...
  if (!/^migration-[a-f0-9]{24}$/u.test(migrationPackId) || !/^bundle-[a-f0-9]{24}$/u.test(bundleId) || !SHA256_PATTERN3.test(sourceTreeHash) || !TARGET_IDENTITY_PATTERN2.test(targetIdentity))
    fail3("MIGRATION_RECEIPT.json has invalid identity fields.");
  ...
  if (ids.some((item) => !STABLE_ID_PATTERN.test(item)))
    fail3("MIGRATION_RECEIPT.json.converted_artifact_ids contains an invalid artifact ID.");
...
function canonicalArtifactIdentity(relativePath, content) {
  ...
  if (relativePath === ".workflow-system/PROJECT_PROFILE.yaml") {
    ...
    try {
      ...
      if (diagnostics.length > 0)
        fail3(`migrated project profile has invalid YAML: ${diagnostics.map((item) => item.message).join("; ")}`);
    ...
    if (metadata.document_id !== canonicalDocumentId(kind, sourcePath, sourceSha))
      fail3("migrated project profile document_id is not bound to its source identity.");
  } else {
    const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/u.exec(content);
    if (!match)
  ...
  return { stableId: `artifact-${sha2564(`${kind}\x00${sourcePath}\x00${sourceSha}`).slice(0, 24)}`, sourceRevision, sourceTreeHash };
}
function validateCurrentTaskShape(root, currentTaskPath, runtimeInstalled) {
  const content = fs5.readFileSync(currentTaskPath, "utf8");
  if (runtimeInstalled) {
    try {
      readCanonicalCurrentTask(root);
      return;
    } catch (error) {
      fail3(`current canonical CURRENT_TASK.md is invalid: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/u.exec(content);
  if (!match)
    fail3("current canonical CURRENT_TASK.md is missing vNext frontmatter.");
  const parsed = parseDocument2(match[1], { uniqueKeys: true });
  const diagnostics = [...parsed.errors, ...parsed.warnings];
  if (diagnostics.length > 0)
    fail3(`current canonical CURRENT_TASK.md frontmatter is invalid: ${diagnostics.map((item) => item.message).join("; ")}`);
...
function profileTargetIdentity(root, profile) {
  ...
  if (!SAFE_ID_PATTERN.test(slug))
    fail3(`PROJECT_PROFILE.yaml.project.slug is not a safe slug: ${slug}`);
  return sha2564(`${normalizeRoot(root)}\x00${slug}`).slice(0, 32);
}
function validateCompletedMigrationProvenance(targetRoot) {
  const root = path6.resolve(targetRoot);
  ...
  const hasState = fs5.existsSync(statePath);
  const hasReceipt = fs5.existsSync(receiptPath);
  ...
  for (const removedPath of state.removed_legacy_files) {
    if (admittedCurrentPaths.has(removedPath))
      continue;
    if (fs5.existsSync(resolveRepoPath2(root, removedPath, `removed legacy path ${removedPath}`)))
      fail3(`completed migration still has a Pack-declared removed legacy path: ${removedPath}`);
  }
  ...
  if (!fs5.existsSync(profilePath) || !fs5.statSync(profilePath).isFile())
    fail3("completed migration provenance is missing PROJECT_PROFILE.yaml.");
  const profile = readYaml(profilePath, "PROJECT_PROFILE.yaml");
  if (profileTargetIdentity(root, profile) !== state.target_identity)
    fail3("completed migration target identity does not match the current project profile/root.");
  const workflowHomeValue = isRecord3(profile.paths) ? profile.paths.workflow_home : undefined;
  ...
  if (workflowHome !== "docs/workflow")
    fail3("completed migration workflow_home is not canonical.");
  ...
  if (!fs5.existsSync(currentTaskPath) || !fs5.statSync(currentTaskPath).isFile())
    fail3("completed migration provenance is missing the canonical CURRENT_TASK.md.");
  ...
  if (!currentTaskRecord || !SHA256_PATTERN3.test(currentTaskRecord.checksum))
    fail3("completed migration install state does not record the canonical CURRENT_TASK.md baseline.");
  ...
  if (migratedFiles.length === 0)
    fail3("completed migration install state contains no converted artifact records.");
  if (!migratedFiles.some((entry) => entry.path === ".workflow-system/PROJECT_PROFILE.yaml"))
    fail3("completed migration install state does not record the converted PROJECT_PROFILE.yaml artifact.");
  const derived = migratedFiles.map((entry) => {
    ...
    if (!fs5.existsSync(fullPath) || !fs5.statSync(fullPath).isFile())
      fail3(`converted migration artifact is missing: ${entry.path}`);
    return { path: entry.path, ...canonicalArtifactIdentity(entry.path, fs5.readFileSync(fullPath, "utf8")) };
  });
  ...
  if (JSON.stringify(derivedIds) !== JSON.stringify(receipt.converted_artifact_ids))
    fail3("MIGRATION_RECEIPT.json converted_artifact_ids do not match current converted artifact identities.");
  ...
  if (derived.some((item) => item.sourceRevision !== revision || item.sourceTreeHash !== treeHash))
    fail3("converted artifacts do not share one migration provenance identity.");
  return {
...
var BOOTSTRAP_GOVERNANCE_SECTIONS = {
  contractsFacts: "Confirmed boundaries",
  decisionsFacts: "Confirmed facts",
  decisionsUnresolved: "Inferred or unknown facts",
  roadmapBaseline: "Design baseline consumed"
};
var DESIGN_PATHS = [
...
function expectRecord4(value, location) {
  if (!isRecord4(value))
    fail4("BOOTSTRAP_SUPPORT_SCHEMA_INVALID", `${location} must be a mapping.`);
  return value;
}
function expectString4(value, location) {
  if (typeof value !== "string" || value.trim().length === 0)
    fail4("BOOTSTRAP_SUPPORT_SCHEMA_INVALID", `${location} must be a non-empty string.`);
  return value.trim();
}
function expectExactKeys4(value, keys, location) {
...
function targetPath(root, relative2) {
  ...
  const resolved = path7.resolve(resolvedRoot, ...normalizeRelative(relative2).split("/"));
  const prefix = resolvedRoot.endsWith(path7.sep) ? resolvedRoot : `${resolvedRoot}${path7.sep}`;
  if (resolved !== resolvedRoot && !resolved.startsWith(prefix))
    fail4("BOOTSTRAP_SUPPORT_PATH_INVALID", `target path escapes root: ${relative2}`);
  return resolved;
}
function readJsonObject2(filePath, location) {
  if (!fs6.existsSync(filePath) || !fs6.statSync(filePath).isFile())
    fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", `${location} is missing.`);
  try {
    return expectRecord4(JSON.parse(fs6.readFileSync(filePath, "utf8")), location);
  } catch (error) {
    if (error instanceof BootstrapSupportError)
      throw error;
    fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", `${location} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
}
function readYamlObject(filePath, location) {
  if (!fs6.existsSync(filePath) || !fs6.statSync(filePath).isFile())
    return {};
  try {
    const value = parse3(fs6.readFileSync(filePath, "utf8"));
    return value === null ? {} : expectRecord4(value, location);
  } catch (error) {
    fail4("BOOTSTRAP_SUPPORT_PROFILE_INVALID", `${location} is invalid: ${error instanceof Error ? error.message : String(error)}`);
  }
}
function fileHash(filePath) {
  return sha2565(fs6.readFileSync(filePath));
}
...
function profileProject(profile) {
  ...
  if (!isRecord4(project) || typeof project.name !== "string" || typeof project.slug !== "string")
    return null;
...
function resolveProject(root, options, receipt) {
  ...
  const name = options.projectName?.trim() || profileIdentity?.name || receipt?.project.name || packageName || path7.basename(path7.resolve(root));
  const slug = options.projectSlug?.trim() || profileIdentity?.slug || receipt?.project.slug || safeSlug(name);
  if (!SAFE_SLUG.test(slug))
    fail4("BOOTSTRAP_SUPPORT_PROFILE_INVALID", `project slug must be lowercase kebab-case: ${slug}`);
  if (profileIdentity && options.projectName && profileIdentity.name !== options.projectName.trim())
    fail4("BOOTSTRAP_SUPPORT_IDENTITY_CONFLICT", "caller project name conflicts with the existing project profile.");
  if (profileIdentity && options.projectSlug && profileIdentity.slug !== options.projectSlug.trim())
    fail4("BOOTSTRAP_SUPPORT_IDENTITY_CONFLICT", "caller project slug conflicts with the existing project profile.");
  if (profile?.paths && isRecord4(profile.paths) && profile.paths.workflow_home !== undefined && profile.paths.workflow_home !== "docs/workflow")
    fail4("BOOTSTRAP_SUPPORT_IDENTITY_CONFLICT", "existing paths.workflow_home is not the canonical vNext workflow home.");
  return { name, slug };
}
...
function mergeGovernanceFacts(existing, caller) {
  const merged = new Map;
  for (const fact of existing) {
    const previous = merged.get(fact.key);
    if (previous && !sameGovernanceFact(previous, fact))
      fail4("BOOTSTRAP_SUPPORT_TARGET_CONFLICT", `existing governance facts conflict for key: ${fact.key}.`);
...
function isVNextMarkerFile(root, relative2) {
  ...
  if (!fs6.existsSync(filePath) || !fs6.statSync(filePath).isFile())
    return false;
  return /vnext|vNext/iu.test(fs6.readFileSync(filePath, "utf8").slice(0, 1600));
}
function isFrozenPath(root, relative2) {
  ...
  if (fs6.existsSync(filePath) && fs6.statSync(filePath).isFile()) {
    const head = fs6.readFileSync(filePath, "utf8").split(/\r?\n/u).slice(0, 20).join(`
`);
...
function readBootstrapReceipt(root) {
  ...
  if (raw.schema_version !== 1 || raw.kind !== "vnext-bootstrap-receipt" || !BOOTSTRAP_MODES.includes(raw.mode) || typeof raw.target_identity !== "string" || typeof project.name !== "string" || typeof project.slug !== "string" || !["codex", "claude", "factory"].includes(raw.host) || typeof source.revision !== "string" || typeof source.tree_hash !== "string" || !HASH64.test(source.tree_hash) || typeof raw.input_fingerprint !== "string" || !HASH64.test(raw.input_fingerprint) || typeof raw.completed_at !== "string" || raw.legacy_compatibility !== "absent" || raw.recovery_boundary !== "in-progress-marker") {
    fail4("BOOTSTRAP_SUPPORT_RECEIPT_INVALID", "BOOTSTRAP_RECEIPT.json identity or schema fields are invalid.");
  }
  ...
  const managed = raw.managed_files.map((item, index) => {
    ...
    const relative2 = normalizeRelative(expectString4(record.path, `BOOTSTRAP_RECEIPT.json.managed_files[${index}].path`));
    ...
    if (relative2.startsWith(".workflow-system/runtime/") || relative2.startsWith(".agents/skills/") || relative2 === ".workflow-system/WORKFLOW_PROTOCOL.md" || relative2 === ".workflow-system/FILE_SCHEMAS.md" || relative2 === ".workflow-system/vnext/SOURCE_CONTRACT.yaml" || relative2 === ".workflow-system/vnext/RUNTIME_CONTRACT.yaml")
      fail4("BOOTSTRAP_SUPPORT_RECEIPT_INVALID", `BOOTSTRAP receipt cannot own Distribution software: ${relative2}`);
...
function validateSkillFile(root, entry) {
  ...
  const filePath = targetPath(root, relative2);
  if (!fs6.existsSync(filePath) || !fs6.statSync(filePath).isFile())
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `required Distribution Skill is missing: ${relative2}`);
  const content = fs6.readFileSync(filePath, "utf8");
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?/u.exec(content);
  if (!match)
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `${relative2} is missing Agent Skill frontmatter.`);
  const document = parseDocument3(match[1], { uniqueKeys: true });
  const diagnostics = [...document.errors, ...document.warnings];
  if (diagnostics.length > 0)
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `${relative2} frontmatter is invalid: ${diagnostics.map((item) => item.message).join("; ")}`);
  const frontmatter = expectRecord4(document.toJS(), `${relative2} frontmatter`);
  if (frontmatter.name !== entry || typeof frontmatter.description !== "string" || !frontmatter.description.trim())
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `${relative2} must declare name=${entry} and a non-empty description.`);
  const contract = expectRecord4(frontmatter.entry_contract, `${relative2}.entry_contract`);
  if (contract.entry !== entry)
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `${relative2}.entry_contract.entry is not canonical.`);
}
function validateInstalledDistributionReadback(root) {
  ...
  if (fs6.existsSync(journalPath))
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "Distribution has an interrupted transaction marker; recover through Distribution before Bootstrap.");
  ...
  if (raw.schema_version !== 1 || raw.kind !== "vibe-governance-distribution-state" || raw.distribution_state !== "vnext" || raw.legacy_compatibility !== "absent" || raw.recovery_boundary !== "distribution-journal")
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "DISTRIBUTION_STATE.json is not a completed vNext Distribution state.");
  ...
  if (!HASH64.test(manifestDigest) || !/^\d+\.\d+\.\d+$/u.test(version))
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "Distribution State version or manifest digest is invalid.");
  if (!Array.isArray(raw.managed_files) || raw.managed_files.length === 0)
  ...
  const managed = raw.managed_files.map((item, index) => {
    ...
    const relative2 = normalizeRelative(expectString4(record.path, `DISTRIBUTION_STATE.json.managed_files[${index}].path`));
    ...
    if (!isDistributionManagedPath(relative2) || !HASH64.test(checksum))
      fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `Distribution State contains an invalid managed path/checksum: ${relative2}`);
    ...
    if (!["protocol", "schema", "skill", "runtime", "config"].includes(category))
      fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `Distribution State contains an unsupported managed category: ${relative2}`);
  ...
  for (const entry of managed) {
    ...
    if (!fs6.existsSync(filePath) || !fs6.statSync(filePath).isFile() || fileHash(filePath) !== entry.checksum)
      fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `Distribution managed artifact read-back failed: ${entry.path}`);
  }
  const required = [
  ...
  for (const relative2 of required) {
    ...
    if (!entry)
      fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", `Distribution State does not admit required artifact: ${relative2}`);
  }
  const runtime = validateVNextRuntimeContract(root, true).runtime_distribution;
  if (runtime.package_version !== version)
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "Distribution State and project-local Runtime versions differ.");
  const protocol = fs6.readFileSync(targetPath(root, ".workflow-system/WORKFLOW_PROTOCOL.md"), "utf8");
  const schema = fs6.readFileSync(targetPath(root, ".workflow-system/FILE_SCHEMAS.md"), "utf8");
  if (!/^kind:\s*vnext-protocol\s*$/imu.test(protocol) || !/schema_version\s*:\s*1/iu.test(protocol))
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "installed vNext Protocol is invalid.");
  if (!/^kind:\s*vnext-file-schema\s*$/imu.test(schema) || !/CURRENT_TASK\.md/iu.test(schema))
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "installed vNext Schema is invalid.");
  const supportTemplate = fs6.readFileSync(targetPath(root, BOOTSTRAP_SUPPORT_TEMPLATE_RELATIVE_PATH), "utf8");
  if (!/^kind:\s*vnext-current-task\s*$/imu.test(supportTemplate) || !supportTemplate.includes("# vNext CURRENT_TASK"))
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_INVALID", "installed Bootstrap support template is invalid.");
  for (const entry of REQUIRED_SKILL_ENTRIES)
    validateSkillFile(root, entry);
  return { distribution_version: version, manifest_digest: manifestDigest, state_content: fs6.readFileSync(statePath, "utf8"), managed_files: managed };
}
...
var PUBLIC_ENTRY_TERMINAL_GUIDANCE = [
...
function renderContracts(project, facts = [], preservedRecordsSection = null) {
  ...
  return [
    ...
    `Project: ${project.name} (${project.slug})`,
    "",
    `## ${BOOTSTRAP_GOVERNANCE_SECTIONS.contractsFacts}`,
    "",
...
function renderDecisions(project, mode, facts = [], draft = false, preservedRecordsSection = null) {
  ...
  return [
    ...
    "",
    `## ${BOOTSTRAP_GOVERNANCE_SECTIONS.decisionsFacts}`,
    "",
    ...confirmed.length > 0 ? confirmed.map((fact) => `- ${fact.key}: ${fact.value} (source: ${fact.source})`) : ["- none"],
    "",
    `## ${BOOTSTRAP_GOVERNANCE_SECTIONS.decisionsUnresolved}`,
    "",
...
function renderRoadmap(project, mode, baseline, preservedBaselineKeys = []) {
  const baselineKeys = [...new Set([...Object.keys(baseline ?? {}), ...preservedBaselineKeys])].sort((left, right) => left.localeCompare(right));
  return ["# ROADMAP.md", "", `Project: ${project.name} (${project.slug})`, "", "## Current boundary", "", `- ${mode} bootstrap completed as an administrative workflow operation.`, "", "## Next boundary", "", "- Prepare a concrete task with confirmed acceptance, exact mutation scope, and minimum-sufficient evidence.", ...baseline ? ["", `## ${BOOTSTRAP_GOVERNANCE_SECTIONS.roadmapBaseline}`, "", ...baselineKeys.map((key) => `- ${key}`)] : [], ""].join(`
`);
}
function renderDesignDocument(filePath, baseline) {
  const key = path7.basename(filePath, ".md");
...
function mergeAssets(...groups) {
  ...
  for (const asset of groups.flat()) {
    if (map.has(asset.path))
      fail4("BOOTSTRAP_SUPPORT_TARGET_CONFLICT", `generated asset path is duplicated: ${asset.path}`);
...
function renderReceipt(mode, targetIdentity, project, host, source, inputFingerprint, assets) {
  return `${JSON.stringify({
    ...
    managed_files: assets.map((asset) => ({ path: asset.path, checksum: sha2565(Buffer.from(asset.content, "utf8")) })).sort((left, right) => left.path.localeCompare(right.path)),
    legacy_compatibility: "absent",
    recovery_boundary: "in-progress-marker"
  }, null, 2)}
`;
...
function classifyBootstrapTargetLocal(root, options = {}) {
  if (fs6.existsSync(targetPath(root, BOOTSTRAP_SUPPORT_MARKER_RELATIVE_PATH)))
  ...
  try {
    ...
    if (isRecord4(profileVNext) && profileVNext.target_identity !== undefined && profileVNext.target_identity !== targetIdentity)
  ...
  if (governanceSignals.some((relative2) => fs6.existsSync(targetPath(root, relative2))))
...
function modeBlockers(root, state, mode, options, baseline, facts, receipt, migration) {
  ...
  if (state === "legacy")
    blockers.push({ code: "MIGRATION_REQUIRED", message: "legacy/compatibility assets are outside Bootstrap; run the offline Migration Pack boundary." });
  ...
  if (["greenfield", "adopt", "realign"].includes(mode) && fs6.existsSync(targetPath(root, CURRENT_TASK_RELATIVE_PATH))) {
    try {
  ...
  if (mode === "adopt") {
    ...
    if (state !== "valid" && !fs6.existsSync(targetPath(root, "docs/adoption/architecture-inventory.md")))
  ...
  if (mode === "realign" && !["valid", "stale", "incomplete", "governed"].includes(state))
...
function verifyReceipt(root, receipt, assets) {
  ...
  const expected = assets.filter((asset) => asset.path !== BOOTSTRAP_SUPPORT_RECEIPT_RELATIVE_PATH).map((asset) => ({ path: asset.path, checksum: sha2565(Buffer.from(asset.content, "utf8")) })).sort((left, right) => left.path.localeCompare(right.path));
  const actual = receipt.managed_files.slice().sort((left, right) => left.path.localeCompare(right.path));
  if (JSON.stringify(expected) !== JSON.stringify(actual))
    fail4("BOOTSTRAP_SUPPORT_READ_BACK_FAILED", "Bootstrap receipt managed_files does not match the governance proposal.");
}
function verifyBootstrapHealth(root, proposal, distributionBefore) {
  ...
  if (distributionAfter.state_content !== distributionBefore.state_content)
    fail4("BOOTSTRAP_SUPPORT_DISTRIBUTION_MUTATED", "Bootstrap changed Distribution State bytes.");
  for (const asset of proposal.assets) {
    const filePath = targetPath(root, asset.path);
    if (!fs6.existsSync(filePath) || !fs6.statSync(filePath).isFile() || fileHash(filePath) !== sha2565(Buffer.from(asset.content, "utf8")))
      fail4("BOOTSTRAP_SUPPORT_READ_BACK_FAILED", `promoted governance asset did not read back identically: ${asset.path}`);
  }
  ...
  if (!receipt || receipt.target_identity !== proposal.target_identity || receipt.mode !== proposal.mode)
    fail4("BOOTSTRAP_SUPPORT_READ_BACK_FAILED", "Bootstrap receipt identity did not read back correctly.");
  ...
  if (["greenfield", "adopt", "realign"].includes(proposal.mode)) {
    ...
    if (current.runtimeState.workflow_status !== "closed" || current.runtimeState.lifecycle_state !== "archived" || current.runtimeState.active_step_status !== "completed" || proposal.mode !== "realign" && current.runtimeState.task_id !== "000")
      fail4("BOOTSTRAP_SUPPORT_READ_BACK_FAILED", "canonical CURRENT_TASK is not a closed + archived Bootstrap baseline.");
  }
}
...
function prepareProposal(options, distribution, classification) {
  const root = path7.resolve(options.targetRoot);
  ...
  if (options.mode === "realign") {
    ...
    assets = assets.filter((asset) => asset.path !== CURRENT_TASK_RELATIVE_PATH && !(preservedMigrationPaths.has(asset.path) && fs6.existsSync(targetPath(root, asset.path))));
  }
  const targetIdentity = computeBootstrapTargetIdentity(root);
  ...
  for (const relative2 of plannedWrites) {
    if (isFrozenPath(root, relative2))
      fail4("BOOTSTRAP_SUPPORT_FROZEN_PATH", `Bootstrap cannot replace a frozen target: ${relative2}`);
    if (!existingIsWorkflowOwned(root, relative2, classification.receipt))
      fail4("BOOTSTRAP_SUPPORT_TARGET_CONFLICT", `target-owned/native asset would be overwritten: ${relative2}`);
  }
  ...
  try {
    ...
    fail4(error instanceof Error && "code" in error ? String(error.code) : "BOOTSTRAP_SUPPORT_PROPOSAL_INVALID", error instanceof Error ? error.message : String(error));
  }
...
function bootstrapProjectTargetLocal(options) {
  ...
  try {
    ...
    try {
      fs6.mkdirSync(path7.dirname(markerPath), { recursive: true });
      fs6.writeFileSync(markerPath, markerValue(proposal), "utf8");
      ...
      if (rollbackVerified && fs6.existsSync(markerPath))
        fs6.rmSync(markerPath, { force: true });
...
function parseCliFlags(argv) {
  ...
  for (let index = 0;index < rest.length; index += 1) {
    const arg = rest[index];
    if (!arg?.startsWith("--"))
      fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", `unexpected argument: ${arg ?? ""}`);
...
function flagString(flags, key, required = false) {
  ...
  if (typeof value === "string" && value.trim())
    return value;
  if (value === true)
    fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", `--${key} requires a value.`);
  if (required)
    fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", `--${key} requires a value.`);
  return;
}
function readStringListFile(filePath) {
  const content = fs6.readFileSync(path7.resolve(filePath), "utf8");
  if (content.trimStart().startsWith("[")) {
    const parsed = JSON.parse(content);
    if (!Array.isArray(parsed) || parsed.some((value) => typeof value !== "string"))
      fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", "changed paths file JSON form must be an array of strings.");
    return parsed;
  }
  return content.split(/\r?\n/u).map((line) => line.trim()).filter(Boolean);
}
function readFactsFile(filePath) {
  const value = JSON.parse(fs6.readFileSync(path7.resolve(filePath), "utf8"));
  if (!Array.isArray(value))
    fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", "facts file must contain an array.");
  return value;
}
...
function readAuthorizationsFile(filePath) {
  const value = JSON.parse(fs6.readFileSync(path7.resolve(filePath), "utf8"));
  if (!Array.isArray(value))
    fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", "conditional authorizations file must contain an array.");
  return value;
}
...
`));
    ...
    if (command !== "prepare")
      fail4("BOOTSTRAP_SUPPORT_INPUT_INVALID", `unknown bootstrap-support command: ${command}`);
    const mode = flagString(flags, "mode", true);
    if (!BOOTSTRAP_MODES.includes(mode))
      fail4("BOOTSTRAP_SUPPORT_MODE_INVALID", "mode must be one of the closed Bootstrap modes.");
...
function verbatim(value, location, maximumLength = 32768) {
  if (typeof value !== "string" || value.trim().length === 0 || value.length > maximumLength || /\0/u.test(value)) {
    fail5("PREPARE_ADAPTER_INPUT_INVALID", `${location} must be non-empty verbatim text of at most ${maximumLength} characters.`);
  }
  return value;
}
function normalizeTaskBasisSource(value, location) {
...
function normalizeTaskBasis(value) {
  ...
  if (!Array.isArray(basis.user_decisions) || basis.user_decisions.length > 64) {
    fail5("PREPARE_ADAPTER_INPUT_INVALID", "task_basis.user_decisions must be a bounded array.");
  }
  ...
  if (new Set(keys).size !== keys.length) {
    fail5("PREPARE_ADAPTER_INPUT_INVALID", "task_basis.user_decisions must not contain duplicate source excerpts.");
  }
  return {
...
function normalizeStepCommands(value, location) {
  if (!Array.isArray(value) || value.length > MAX_ITEMS) {
    fail5("PREPARE_ADAPTER_INPUT_INVALID", `${location} must be a bounded array.`);
  }
  return value.map((item, index) => {
...
function commandTransformationKind(command) {
  return command.expected_repo_writes !== "none" && command.expected_repo_writes.some((item) => item.includes("*")) ? "inherently-broad" : "localized";
}
...
function normalizeSemanticDraft(input) {
  ...
  if (duplicatedDecisions.length > 0) {
    fail5("PREPARE_ADAPTER_INPUT_INVALID", "a design decision cannot be both decided and unresolved.");
  }
  ...
  for (const step of input.implementation_steps) {
    ...
    for (const [commandIndex, command] of step.commands.entries()) {
      ...
      if (outsideStepScope.length > 0) {
        fail5("COMMAND_FOOTPRINT_BLOCKED", `step ${step.id} command "${command.command}" writes outside the step mutation scope: ${outsideStepScope.join(", ")}.`);
      }
      ...
      if (result.status !== "pass") {
        fail5("COMMAND_FOOTPRINT_BLOCKED", `step ${step.id} command "${command.command}" has no executable repository-write plan: ${result.blockers.join(" ")}`);
      }
    }
  }
}
...
function currentMatchesSemanticDraft(root, current, semantic) {
  ...
  return current.runtimeState.active_step_id === semantic.implementation_steps[0].id && basisMatches && sameValue(readDraftDefinitionFromBody(current.body), semanticDraftDefinition(semantic)) && current.runtimeState.business_evidence_version === 1 && !!current.runtimeState.evidence_plan_revision && sameValue(current.runtimeState.claim_evidence, claimEvidence(semantic));
}
...
function record2(value, location) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", `${location} must be an object.`);
  }
  return value;
}
function exactKeys2(value, expected, location) {
  const allowed = new Set(expected);
  const missing = expected.filter((key) => !(key in value));
  const unexpected = Object.keys(value).filter((key) => !allowed.has(key));
  if (missing.length > 0 || unexpected.length > 0) {
...
function text2(value, location, maximumLength = 4096) {
  ...
  const normalized = value.trim();
  if (!normalized || normalized.length > maximumLength || /[\r\n]/u.test(normalized)) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", `${location} must be one non-empty line of at most ${maximumLength} characters.`);
  }
  return normalized;
}
...
function pathList(value, location, allowEmpty, allowGlob = false) {
  if (!Array.isArray(value) || value.length > MAX_ITEMS2 || !allowEmpty && value.length === 0) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", `${location} must be a bounded${allowEmpty ? "" : " non-empty"} array.`);
  }
...
function digest3(value) {
  return crypto8.createHash("sha256").update(JSON.stringify(value)).digest("hex");
}
function idempotencyKey(prefix, value) {
  return `${prefix}-${digest3(value).slice(0, 48)}`;
}
...
function assertExecutableTask(current) {
  if (current.runtimeState.workflow_status === "draft" && current.runtimeState.lifecycle_state === "active") {
    fail6("DRAFT_NOT_EXECUTABLE", "execute-step requires an explicitly confirmed task; the current task is draft + active.");
  }
  if (current.runtimeState.workflow_status !== "active" || current.runtimeState.lifecycle_state !== "active") {
    fail6("TASK_STATE_NOT_ACTIVE", "execute-step requires active + active.");
  }
  if (current.runtimeState.resume_requires_review) {
    fail6("RESUME_REVIEW_REQUIRED", "execute-step is blocked by the current resume-review gate.");
  }
}
function parseScopeList(value, location) {
  if (!value)
    fail6("TASK_STEP_METADATA_INCOMPLETE", `${location} is missing.`);
  const items = value.split(",").map((item) => item.replace(/`/gu, "").trim()).filter(Boolean);
  if (items.length === 0 || new Set(items).size !== items.length) {
    fail6("TASK_STEP_METADATA_INCOMPLETE", `${location} must be a non-empty list without duplicates.`);
  }
  return items.map((item, index) => normalizePath(item, `${location}[${index}]`, true));
}
function parseValidationList(value, location) {
  if (!value)
    fail6("TASK_STEP_METADATA_INCOMPLETE", `${location} is missing.`);
...
function parsePlannedCommands(current, stepId) {
...
`);
  ...
  for (let index = 0;index < lines.length; index += 1) {
    ...
    if (!expected || !transformation) {
      fail6("TASK_STEP_COMMAND_PLAN_INVALID", `planned command "${planned[1].trim()}" is missing canonical write-footprint metadata.`);
    }
    ...
    if (expectedWrites !== "none" && expectedWrites.length === 0) {
      fail6("TASK_STEP_COMMAND_PLAN_INVALID", `planned command "${planned[1].trim()}" has an empty write footprint.`);
    }
    commands.push({
      command: planned[1].trim(),
      expected_repo_writes: expectedWrites,
      transformation_kind: transformation[1]
    });
    index += 2;
  }
  if (new Set(commands.map((item) => item.command)).size !== commands.length) {
    fail6("TASK_STEP_COMMAND_PLAN_INVALID", `step ${stepId} contains duplicate planned commands.`);
  }
  return commands;
}
function currentStepPlan(current) {
  ...
  if (!resolution.current.metadata_complete || !resolution.current.purpose || !resolution.current.review_checkpoint) {
    fail6("TASK_STEP_METADATA_INCOMPLETE", `step ${resolution.current.id} is not executable because its canonical metadata is incomplete.`);
  }
  return {
...
function assertCommandPlansAdmitted(current, stepPlan) {
  const scope = parseMutationScope(current.body, current.sourceTuple.revision);
  for (const command of stepPlan.commands) {
    ...
    if (outsideStep.length > 0) {
      fail6("COMMAND_FOOTPRINT_BLOCKED", `planned command "${command.command}" writes outside current step scope: ${outsideStep.join(", ")}.`);
    }
    const result = evaluateCommandWriteFootprint(scope, {
      command: command.command,
      expected_write_footprint: {
    ...
    if (result.status !== "pass") {
      fail6("COMMAND_FOOTPRINT_BLOCKED", `planned command "${command.command}" is not executable: ${result.blockers.join(" ")}`);
    }
  }
}
function assertExactCommandWritesCovered(stepPlan, candidatePaths) {
  ...
  if (missing.length > 0) {
    fail6("COMMAND_FOOTPRINT_BLOCKED", `candidate_paths must include every exact planned command write: ${[...new Set(missing)].join(", ")}.`);
  }
}
...
function assertCurrentReceipt(current, stepPlan, receipt) {
  if (receipt.task_id !== current.runtimeState.task_id || receipt.document_id !== current.sourceTuple.document_id) {
    fail6("EXECUTE_PREFLIGHT_IDENTITY_CONFLICT", "preflight receipt does not identify the current task document.");
  }
  if (receipt.source_revision !== current.sourceTuple.revision) {
    ...
    if (!expectedRepairBookkeeping) {
      fail6("EXECUTE_PREFLIGHT_STALE", "CURRENT_TASK changed after preflight; run preflight-step again before editing or committing.");
    }
  }
  ...
  if (receipt.kind === "execute-step-repair-preflight/v1") {
    ...
    fail6("EXECUTE_PREFLIGHT_STALE", "the Runtime-owned change set identity changed after preflight.");
  }
}
function findingAdmissionWaveId(reviewId) {
  return `finding-wave-${digest3(reviewId).slice(0, 32)}`;
}
...
function beginRepair(root, input, options = {}) {
  ...
  exactKeys2(source, ["candidate_paths"], "begin-repair input");
  ...
  if (!pending || pending.verdict !== "findings") {
    fail6("REVIEW_FINDINGS_REQUIRED", "begin-repair requires the current durable review result to contain findings.");
  }
  ...
  for (const candidate of pending.findings) {
    ...
    if (result.status !== "success" && result.status !== "no-op") {
      fail6("FINDING_ADMISSION_BLOCKED", result.message);
    }
    if (!options.dryRun)
  ...
  for (const fingerprint of fingerprints) {
    ...
    if (!options.dryRun && (!finding || !["admitted", "in-progress"].includes(finding.status))) {
      fail6("FINDING_ADMISSION_REQUIRED", `review finding ${fingerprint} is not repairable.`);
    }
    if (!options.dryRun && finding.repair_attempts >= finding.max_repair_attempts) {
      fail6("REPAIR_BUDGET_EXHAUSTED", `finding ${fingerprint} has exhausted its repair budget.`);
    }
  }
...
function evidenceContext(root, input) {
  exactKeys2(record2(input, "evidence-context input"), [], "evidence-context input");
  const current = readCanonicalCurrentTask(root);
  assertExecutableTask(current);
  assertBusinessEvidenceVersion(current);
...
function preflightStep(root, input) {
  const source = record2(input, "preflight-step input");
  ...
  if (coverage && !hasPrerequisites && captureReviewTarget(root, coverage.target.entries.map((entry) => entry.path)).revision !== coverage.target.revision)
    fail6("REVIEW_TARGET_STALE", "Unrecorded changes cannot refresh the cumulative baseline.");
  if (!coverage || candidatePaths.some((p) => !coverage.base.entries.some((entry) => entry.path === p)) || hasPrerequisites || current.runtimeState.step_attempts?.[stepPlan.step.id]?.attempts.at(-1)?.status === "ready") {
    ...
    if (!["success", "no-op"].includes(registration.status))
      fail6("PREFLIGHT_BLOCKED", registration.message);
...
function normalizeCommandResults(value) {
  if (!Array.isArray(value) || value.length > MAX_ITEMS2) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", "command_results must be a bounded array.");
  }
  ...
  if (new Set(results.map((item) => item.command)).size !== results.length) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", "command_results must not contain duplicate commands.");
  }
  return results;
}
function normalizeValidationResults(value) {
  if (!Array.isArray(value) || value.length === 0 || value.length > MAX_ITEMS2) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", "validation_results must be a bounded non-empty array.");
  }
  const results = value.map((item, index) => {
  ...
  if (new Set(results.map((item) => item.validation)).size !== results.length) {
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", "validation_results must not contain duplicate validations.");
  }
  return results;
}
...
function updateClaimEvidence(current, evidence) {
  ...
  for (const item of evidence) {
    ...
    if (slot.prerequisite_receipt)
      fail6("CLAIM_EVIDENCE_PLAN_CONFLICT", "consumed prerequisite evidence is immutable.");
...
function assertExactResultSet(actual, expected, location) {
  const missing = expected.filter((item) => !actual.includes(item));
  const unexpected = actual.filter((item) => !expected.includes(item));
  if (missing.length > 0 || unexpected.length > 0) {
    fail6("EXECUTE_RESULT_PLAN_CONFLICT", `${location} must match the current step plan; missing=[${missing.join(", ")}], unexpected=[${unexpected.join(", ")}].`);
  }
}
function assertObservedWithinExpected(command, observed) {
  if (command.expected_repo_writes === "none") {
    if (observed.length > 0) {
      fail6("COMMAND_OBSERVED_WRITE_BLOCKED", `command "${command.command}" was planned as write-free but observed repository writes: ${observed.join(", ")}.`);
    }
    return;
  }
...
function assertCommandResults(current, stepPlan, results) {
  ...
  for (const result of results) {
    ...
    if (audit.status !== "pass") {
      fail6("COMMAND_MUTATION_AUDIT_BLOCKED", `command "${result.command}" failed Runtime mutation audit: ${audit.blockers.join(" ")}`);
    }
  }
}
function assertValidationResults(stepPlan, results) {
  assertExactResultSet(results.map((item) => item.validation), stepPlan.validation, "validation_results");
}
function allEvidenceRefs(commands, validations, acceptance) {
  return [...new Set([
...
function verifyReadBack(root, result, options) {
  ...
  const readBack = readCanonicalCurrentTask(root);
  if (!result.read_back_verified || result.resulting_revision !== readBack.sourceTuple.revision) {
    fail6("EXECUTE_ADAPTER_READ_BACK_FAILED", "execute-step adapter could not verify the committed canonical CURRENT_TASK revision.");
  }
  return result;
}
function semanticNoOp2(current, key, message, options) {
  const state = current.runtimeState;
  return {
    status: "no-op",
    operation_kind: "task-state-transaction",
...
function applyRepairAttempts(root, current, receipt, evidenceRefs, keySeed, options) {
  ...
  const waveId = receipt.kind === "execute-step-repair-preflight/v1" ? receipt.repair_wave_id : `repair-wave-${digest3(keySeed).slice(0, 24)}`;
  let fresh = current;
  for (const fingerprint of fingerprints) {
    const finding = fresh.runtimeState.findings.find((item) => item.fingerprint === fingerprint);
    if (!finding || !["admitted", "in-progress"].includes(finding.status)) {
      fail6("FINDING_ADMISSION_REQUIRED", `repair finding ${fingerprint} is no longer admitted.`);
    }
...
function recordStepResult(root, input, options = {}) {
  const source = record2(input, "record-step-result input");
  exactKeys2(source, [
    "preflight_receipt",
  ...
  if (source.blocker_kind !== undefined && !["environment", "unknown"].includes(String(source.blocker_kind)))
    fail6("EXECUTE_ADAPTER_INPUT_INVALID", "blocker_kind must be environment or unknown.");
  ...
  if (digest3(receipt.review_base.entries.map((item) => item.path)) !== digest3(expectedBasePaths)) {
  ...
  if (digest3([...actualChangedPaths].sort()) !== digest3(detectedChangedPaths)) {
  ...
  if (outcome === "blocked" && ![...commandResults, ...validationResults].some((item) => item.status === "failed" || item.status === "blocked")) {
    fail6("EXECUTE_RESULT_BLOCKED", "blocked requires at least one failed or blocked command or validation result.");
  }
  if (receipt.kind === "execute-step-repair-preflight/v1") {
    const pending = current.runtimeState.pending_review_result;
    const priorExecution = pending && current.runtimeState.execution_log.map((item) => ("action" in item) ? item : cumulativeReviewExecution(current, item)).find((item) => !("action" in item) && item.idempotency_key === pending.execution_id);
    if (!pending || !priorExecution?.execution_result || pending.review_id !== receipt.review_id || pending.change_set_id !== receipt.change_set_id || priorExecution.execution_result.change_set_id !== receipt.change_set_id || digest3(priorExecution.execution_result.review_target.entries.map((item) => item.path)) !== digest3(receipt.review_target_paths)) {
      fail6("REVIEW_TARGET_CONFLICT", "repair result no longer binds the Runtime-recorded reviewed change set.");
    }
  }
  ...
  if (receipt.mode === "repair") {
    ...
    if (current.runtimeState.active_step_id !== receipt.step_id || stepPlanRevision(stepPlan) !== receipt.plan_revision) {
      fail6("EXECUTE_PREFLIGHT_STALE", "repair bookkeeping changed the active step plan unexpectedly.");
    }
  }
  let status;
  if (outcome === "blocked")
    status = "blocked";
  else if (receipt.mode === "repair" || stepPlan.step.review_checkpoint === "not-required")
    status = "completed";
  else
    status = "in-progress";
...
function resolveVerifiedFindings(root, current, receipt, options) {
  for (const fingerprint of receipt.admitted_fingerprints) {
    ...
    const finding = fresh.runtimeState.findings.find((item) => item.fingerprint === fingerprint);
    if (!finding)
      fail6("FINDING_NOT_FOUND", `review receipt finding ${fingerprint} is not in the current queue.`);
...
function completeReviewedStep(root, input, options = {}) {
  ...
  const replay = current.runtimeState.execution_log.find((item) => {
    ...
    return item.idempotency_key === key && hasAppliedProposal2(current, key);
  });
  if (replay && !("action" in replay)) {
    return semanticNoOp2(current, replay.idempotency_key, "This exact reviewed-step completion was already committed.", options);
  }
  const pending = current.runtimeState.pending_review_result;
  let reviewReceipt;
  if (pending) {
    ...
    fail6("CLEAN_REVIEW_REQUIRED", "complete-reviewed-step requires a canonical pending clean review result.");
  }
  const resultKey = idempotencyKey("execute-reviewed-step", { step_id: stepId, review_receipt: reviewReceipt, note });
  if (hasAppliedProposal2(current, resultKey)) {
    return semanticNoOp2(current, resultKey, "This exact reviewed-step completion was already committed.", options);
  }
  assertExecutableTask(current);
  ...
  if (repairLogs.length > 0) {
    if (reviewReceipt.cycle_phase !== "verification") {
      fail6("REVIEW_VERIFICATION_REQUIRED", "a repaired step requires a verification review receipt.");
    }
    ...
    if (repairTargets.length !== 1 || repairTargets[0] !== reviewReceipt.change_set_id) {
      fail6("REPAIR_CHANGE_SET_CONFLICT", "verification must cover the exact repaired logical change set.");
    }
  } else if (reviewReceipt.cycle_phase !== "discovery") {
    fail6("REVIEW_PHASE_INVALID", "an unrepaired step requires a discovery review receipt.");
  }
...
function readSemanticStdin2(command) {
  const raw = !process.stdin.isTTY ? fs8.readFileSync(0, "utf8") : "";
  if (!raw.trim())
    throw new Error(`${command} requires semantic JSON on stdin.`);
  try {
    return JSON.parse(raw);
...
function validateInstalledRuntime2(root) {
  const runtimeManifest = path9.join(path9.resolve(root), ...VNEXT_RUNTIME_PACKAGE_RELATIVE_PATH.split("/"), "package.json");
  if (fs8.existsSync(runtimeManifest))
    validateVNextRuntimeContract(root, true);
}
...
function record3(value, location) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    fail7("REVIEW_ADAPTER_INPUT_INVALID", `${location} must be an object.`);
  return value;
}
function exactKeys3(value, expected, location) {
  const allowed = new Set(expected);
  const missing = expected.filter((key) => !(key in value));
  const unexpected = Object.keys(value).filter((key) => !allowed.has(key));
  if (missing.length > 0 || unexpected.length > 0) {
...
function text3(value, location, maximumLength = 4096) {
  ...
  const normalized = value.trim();
  if (!normalized || normalized.length > maximumLength || /[\r\n]/u.test(normalized)) {
    fail7("REVIEW_ADAPTER_INPUT_INVALID", `${location} must be one non-empty line of at most ${maximumLength} characters.`);
  }
  return normalized;
}
function textList3(value, location, allowEmpty) {
...
function digest4(value) {
  return crypto9.createHash("sha256").update(JSON.stringify(value)).digest("hex");
}
...
function latestRecordedExecution(current) {
  ...
  if (!latest.change_set_id || !latest.execution_result?.review_target) {
    fail7("REVIEW_TARGET_REQUIRED", "the latest execute-step result does not contain a Runtime-owned review target.");
  }
  if (latest.execution_result.change_set_id !== latest.change_set_id) {
    fail7("REVIEW_TARGET_CONFLICT", "the latest execution change-set identity is inconsistent.");
  }
  assertReviewExecutionEligible(current, latest);
  return cumulativeReviewExecution(current, latest);
}
function stepScope(value, location) {
  if (!value)
    fail7("TASK_STEP_METADATA_INCOMPLETE", `${location} is missing.`);
  const items = value.split(",").map((item) => item.replace(/`/gu, "").trim()).filter(Boolean);
  if (items.length === 0 || new Set(items).size !== items.length)
    fail7("TASK_STEP_METADATA_INCOMPLETE", `${location} is invalid.`);
  return items;
}
function validationList(value, location) {
  if (!value)
    fail7("TASK_STEP_METADATA_INCOMPLETE", `${location} is missing.`);
...
function assertReviewableTask(current) {
  if (current.runtimeState.workflow_status !== "active" || current.runtimeState.lifecycle_state !== "active") {
    fail7("TASK_STATE_NOT_ACTIVE", "review-change requires active + active.");
  }
  if (current.runtimeState.resume_requires_review)
...
function reviewContext(root, input) {
  const source = record3(input, "review-context input");
  exactKeys3(source, [], "review-context input");
  const current = readCanonicalCurrentTask(root);
  ...
  if (currentTarget.revision !== execution.execution_result.review_target.revision) {
    fail7("REVIEW_TARGET_STALE", "product files changed after the latest execution result was recorded.");
  }
  if (current.runtimeState.pending_review_result?.execution_id === execution.idempotency_key && current.runtimeState.pending_review_result.verdict !== "blocked") {
    fail7("REVIEW_ALREADY_RECORDED", "the latest execution already has a durable clean or findings review result.");
  }
  ...
  if (!resolution.current.metadata_complete || !resolution.current.purpose) {
    fail7("TASK_STEP_METADATA_INCOMPLETE", `step ${resolution.current.id} is not reviewable because its metadata is incomplete.`);
  }
  const scope = parseMutationScope(current.body, current.sourceTuple.revision);
  const admitted = phase === "verification" ? current.runtimeState.findings.filter((item) => item.review_cycle_id === current.runtimeState.review_cycle.id && ["admitted", "in-progress"].includes(item.status)) : [];
  const definition = readDraftDefinitionFromBody(current.body);
  const receipt = {
...
function normalizeContextReceipt(value) {
  const source = record3(value, "context_receipt");
  exactKeys3(source, ["kind", "task_id", "document_id", "source_revision", "step_id", "execution_id", "cycle_id", "cycle_phase", "admitted_fingerprints"], "context_receipt");
...
function assertCurrentContext(root, current, receipt) {
  if (receipt.task_id !== current.runtimeState.task_id || receipt.document_id !== current.sourceTuple.document_id)
    fail7("REVIEW_CONTEXT_STALE", "review context belongs to a different task document.");
  if (receipt.source_revision !== current.sourceTuple.revision)
    fail7("REVIEW_CONTEXT_STALE", "CURRENT_TASK changed after review context was issued.");
  if (receipt.step_id !== current.runtimeState.active_step_id || receipt.cycle_id !== current.runtimeState.review_cycle.id)
    fail7("REVIEW_CONTEXT_STALE", "active step or review cycle changed after review context was issued.");
  const latest = latestRecordedExecution(current);
  if (latest.idempotency_key !== receipt.execution_id)
    fail7("REVIEW_CONTEXT_STALE", "recorded execution changed after review context was issued.");
  ...
  if (currentTarget.revision !== latest.execution_result.review_target.revision) {
    fail7("REVIEW_TARGET_STALE", "product files changed after review context was issued.");
  }
  const expectedPhase = latest.mode === "repair" ? "verification" : "discovery";
  if (receipt.cycle_phase !== expectedPhase)
    fail7("REVIEW_CONTEXT_STALE", "review phase changed after review context was issued.");
  ...
  if (open.length !== receipt.admitted_fingerprints.length || open.some((item) => !receipt.admitted_fingerprints.includes(item))) {
    fail7("REVIEW_CONTEXT_STALE", "admitted finding set changed after review context was issued.");
  }
  return cumulativeReviewExecution(current, latest);
}
function assertRecordedTargetCurrent(root, current, receipt) {
  ...
  if (!execution?.execution_result || !execution.change_set_id || execution.execution_result.change_set_id !== execution.change_set_id) {
    fail7("REVIEW_TARGET_CONFLICT", "review result no longer binds its Runtime-recorded execution target.");
  }
  const currentTarget = captureReviewTarget(root, execution.execution_result.review_target.entries.map((item) => item.path));
  if (currentTarget.revision !== execution.execution_result.review_target.revision) {
    fail7("REVIEW_TARGET_STALE", "product files changed after the recorded execution target.");
  }
  return execution;
}
function normalizeFinding(value, index, current) {
  const source = record3(value, `findings[${index}]`);
  exactKeys3(source, ["category", "file", "failure_condition", "required_behavior", "root_cause_status", "evidence_refs"], `findings[${index}]`);
  ...
  if (decision.status !== "pass" || !admittedByStep) {
    fail7("REVIEW_FINDING_SCOPE_BLOCKED", `finding path ${candidate.file} cannot be repaired within the confirmed current-step scope.`);
  }
  return {
    fingerprint: `finding-${digest4({
      task_id: current.runtimeState.task_id,
      file: candidate.file,
      failure_condition: candidate.failure_condition,
...
function verifyReadBack2(root, result, reviewId, options) {
  ...
  if (!result.read_back_verified || current.runtimeState.pending_review_result?.review_id !== reviewId || result.resulting_revision !== current.sourceTuple.revision) {
    fail7("REVIEW_ADAPTER_READ_BACK_FAILED", "review-change adapter could not verify the durable pending review result.");
  }
  return result;
}
function semanticNoOp3(current, key, message, options) {
  const state = current.runtimeState;
  return {
    status: "no-op",
    operation_kind: "task-state-transaction",
...
function recordReviewResult(root, input, options = {}) {
  const source = record3(input, "record-review-result input");
  exactKeys3(source, ["context_receipt", "verdict", "findings", "unresolved_fingerprints", "evidence_refs", "blocker", ...source.test_assessment === undefined ? [] : ["test_assessment"]], "record-review-result input");
  const receipt = normalizeContextReceipt(source.context_receipt);
  if (!["clean", "findings", "blocked"].includes(String(source.verdict)))
    fail7("REVIEW_ADAPTER_INPUT_INVALID", "verdict must be clean, findings, or blocked.");
  ...
  if (!Array.isArray(source.findings) || source.findings.length > MAX_ITEMS3)
    fail7("REVIEW_ADAPTER_INPUT_INVALID", "findings must be a bounded array.");
  const findings = source.findings.map((item, index) => normalizeFinding(item, index, current));
  if (new Set(findings.map((item) => item.fingerprint)).size !== findings.length)
    fail7("REVIEW_ADAPTER_INPUT_INVALID", "findings must not contain duplicates.");
  ...
  if (unresolved.some((item) => !receipt.admitted_fingerprints.includes(item)))
    fail7("REVIEW_ADAPTER_INPUT_INVALID", "unresolved_fingerprints must be drawn from the Runtime review context.");
  const evidenceRefs = textList3(source.evidence_refs, "evidence_refs", false);
  let blocker;
  if (source.blocker === null)
    blocker = null;
  else {
    const raw = record3(source.blocker, "blocker");
  ...
  if (verdict === "findings" && (findings.length === 0 && unresolved.length === 0 || blocker !== null))
    fail7("REVIEW_ADAPTER_INPUT_INVALID", "findings verdict requires a finding and no blocker.");
  if (verdict === "blocked" && (findings.length > 0 || unresolved.length > 0 || blocker === null))
    fail7("REVIEW_ADAPTER_INPUT_INVALID", "blocked requires only a blocker.");
  ...
  const reviewId = `review-${digest4({ receipt, verdict: finalVerdict, findings: finalFindings, unresolved: finalUnresolved, evidenceRefs, blocker: finalBlocker }).slice(0, 40)}`;
  const reviewResult = {
  ...
  const resultKey = `review-result-${digest4(reviewResult).slice(0, 48)}`;
  if (current.runtimeState.pending_review_result?.review_id === reviewId) {
    ...
    if (digest4(durable) !== digest4(reviewResult))
      fail7("REVIEW_REPLAY_CONFLICT", "the durable review id is bound to different review semantics.");
...
} else if (EXECUTE_STEP_ADAPTER_COMMANDS.includes(args[0])) {
  runner = runExecuteStepAdapterCli(args);
} else if (REVIEW_CHANGE_ADAPTER_COMMANDS.includes(args[0])) {
