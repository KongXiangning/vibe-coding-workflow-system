...
const BOOTSTRAP_GOVERNANCE_SECTIONS = {
...
/** Existing canonical documents are durable state; caller facts are an explicit overlay. */
function mergeGovernanceFacts(existing: readonly BootstrapSupportFact[], caller: readonly BootstrapSupportFact[]): BootstrapSupportFact[] {
  const merged = new Map<string, BootstrapSupportFact>();
  for (const fact of existing) {
    const previous = merged.get(fact.key);
    if (previous && !sameGovernanceFact(previous, fact)) fail('BOOTSTRAP_SUPPORT_TARGET_CONFLICT', `existing governance facts conflict for key: ${fact.key}.`);
    merged.set(fact.key, fact);
  }
  for (const fact of caller) merged.set(fact.key, fact);
