...
stop_conditions:
  ...
  - .workflow-system/PROJECT_PROFILE.yaml、AGENTS.md 与现有治理文档相互冲突，无法判断谁是事实源
output:
  - 对账后的 workflow 资产清单
  - 更新后的 `.workflow-system/PROJECT_PROFILE.yaml`
  - 更新后的 `AGENTS.md`；已有 `CLAUDE.md` 保持原样
...
verification:
  ...
  - AGENTS.md 与 .workflow-system/PROJECT_PROFILE.yaml 对同一套规则达成一致；已有 CLAUDE.md
    不在维护范围
...
- 所有 workflow 管理文档都落到 `docs/workflow/`、`docs/designs/`、`docs/adoption/` 的正确分类目录
- runtime skill prune 只影响 `workflow-system-` 前缀目录
- `AGENTS.md` 与 `.workflow-system/PROJECT_PROFILE.yaml` 对同一套规则达成一致；已有 `CLAUDE.md` 不在维护范围
- 如仍存在需要人工决定的差异，明确列出而不是静默跳过
...
6. `AGENTS.md` 与 `.workflow-system/PROJECT_PROFILE.yaml` 是当前维护面。已有 `CLAUDE.md` 只作为目标项目自有历史文件保留，不读取、不覆盖、不删除。
