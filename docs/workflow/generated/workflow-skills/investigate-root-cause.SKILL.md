...
根因调查前先用 `validate --summary` 和只读 `task-context` 固定当前状态、步骤、义务、依赖与门禁；用 `task-read` 按精确 execution/event/report/history 引用读取失败事实并跟完 continuation。不得展开无界 RuntimeState、用最近 N 条历史代替依赖，或把读取收据当作修复权限、执行证明或 clean 结论。
