...
| remaining expert/internal/intake operations（下一 boundary） | 按明确 authority、evidence、rollback boundary 实现剩余 `validate-change` / `sync-state` / `capture-work-item` 等 surface | 不恢复 legacy compatibility fallback 或扩大 daily intent 数量 |
