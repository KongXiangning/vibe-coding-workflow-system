...
- P8 review follow-up is resolved: `WORKFLOW_PROTOCOL.md` §16 now documents `blocks-merge` as the default blocker level for the unbound project-level slots in the base matrix, `scripts/validation-model.ts` now rejects project-layer entries that attempt to use `blocker_level: blocks-generator`, and `test/validation-model.test.ts` covers both boundaries.
