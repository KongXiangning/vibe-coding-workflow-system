...
When the implementation plan and this inventory diverge on dynamic execution state, this inventory is authoritative for current phase status, implementation artifacts, review follow-ups, and test coverage.
...
| P9 | [run-validation.ts](/e:/coding/github/gstack/scripts/run-validation.ts), [check-freshness.ts](/e:/coding/github/gstack/scripts/check-freshness.ts), [workflow-system-ci.yml](/e:/coding/github/gstack/.github/workflows/workflow-system-ci.yml) | Protocol-level validation runner with layer precedence, content-based generator freshness checks, and workflow-system CI | [run-validation.test.ts](/e:/coding/github/gstack/test/run-validation.test.ts), [workflow-core.test.ts](/e:/coding/github/gstack/test/workflow-core.test.ts) | Complete |
| P10 | [workflow-runtime.ts](/e:/coding/github/gstack/scripts/workflow-runtime.ts), [WORKFLOW_PROTOCOL.md](/e:/coding/github/gstack/WORKFLOW_PROTOCOL.md) §17, [package.json](/e:/coding/github/gstack/package.json) `workflow:*` scripts | Repo-local runtime health report, export manifest JSON with machine-readable `package_json_contract`, and isolated host-sync plans under `.claude/skills/workflow-system-*`, `.agents/skills/workflow-system-*`, or `.factory/skills/workflow-system-*` with distinct planned/applied orphan pruning semantics | [workflow-runtime.test.ts](/e:/coding/github/gstack/test/workflow-runtime.test.ts) | Complete |
| P11 | [WORKFLOW_PROTOCOL.md](/e:/coding/github/gstack/WORKFLOW_PROTOCOL.md) §18, [FILE_SCHEMAS.md](/e:/coding/github/gstack/FILE_SCHEMAS.md), [ARCHITECTURE.md](/e:/coding/github/gstack/ARCHITECTURE.md), [workflow-doc-contracts.ts](/e:/coding/github/gstack/scripts/workflow-doc-contracts.ts), [run-validation.ts](/e:/coding/github/gstack/scripts/run-validation.ts), [ROADMAP.md.tmpl](/e:/coding/github/gstack/templates/docs/ROADMAP.md.tmpl), [BASELINES.md.tmpl](/e:/coding/github/gstack/templates/docs/BASELINES.md.tmpl), [DECISIONS.md.tmpl](/e:/coding/github/gstack/templates/docs/DECISIONS.md.tmpl) | [ROADMAP.md](/e:/coding/github/gstack/generated/workflow-docs/ROADMAP.md), [BASELINES.md](/e:/coding/github/gstack/generated/workflow-docs/BASELINES.md), [DECISIONS.md](/e:/coding/github/gstack/generated/workflow-docs/DECISIONS.md) | [gen-workflow-docs.test.ts](/e:/coding/github/gstack/test/gen-workflow-docs.test.ts), [bootstrap-project-governance.test.ts](/e:/coding/github/gstack/test/bootstrap-project-governance.test.ts), [run-validation.test.ts](/e:/coding/github/gstack/test/run-validation.test.ts) | Complete (Incubation scope) |
...
- P7a review follow-up is resolved: `scripts/bootstrap-project-governance.ts` now seeds the four minimum `A4` validation slots with `blocks-merge` defaults so bootstrap matches the current validation model, and `test/bootstrap-project-governance.test.ts` now covers the placeholder-preserved task identity path in addition to absent and materialized states.
...
- P7b review follow-up is resolved: `generated/workflow-skills/create-current-task.SKILL.md` now aligns with the committed `CURRENT_TASK.md` / `FILE_SCHEMAS.md` contract by requiring `已确认决策` and `待确认问题` instead of the obsolete `决策分类` section, and `test/gen-workflow-skills.test.ts` now asserts that alignment directly.
...
- P8 defines the project-level validation model in `WORKFLOW_PROTOCOL.md` §16 with six subsections: validation layers, blocker levels, layer precedence, validation matrix contract, freshness as protocol-level gates, and separation of concerns. The `scripts/validation-model.ts` module provides types, constants, matrix parsing, layer partitioning, and report building. `PROJECT_PROFILE.yaml` now carries a `validation.matrix` with 8 protocol-level and 4 project-level entrypoints. The implementation-side verification artifact for this phase remains `test/validation-model.test.ts`; later aggregate workflow scripts belong to subsequent phases, not to P8 itself.
- P8 review follow-up is resolved: `WORKFLOW_PROTOCOL.md` §16 now documents `blocks-merge` as the default blocker level for the unbound project-level slots in the base matrix, `scripts/validation-model.ts` rejects project-layer entries that attempt to use `blocker_level: blocks-generator` and rejects protocol-layer workflow-system entries that are demoted below `blocks-merge`, and `test/validation-model.test.ts` covers both boundaries.
- P9 wires protocol-level checks via `scripts/run-validation.ts` (matrix-driven runner with `--layer`, `--blocker-level`, `--json`, `--dry-run` flags) and `scripts/check-freshness.ts` (materializes fresh outputs in an isolated temporary root and compares them against committed generated artifacts). `validate:protocol`, `validate:all`, and `validate:freshness` scripts are available in `package.json`, and `.github/workflows/workflow-system-ci.yml` now runs protocol validation, freshness gates, and the workflow-system test suite in CI. `test:workflow-all` now includes `test:run-validation`.
- P10 adds `scripts/workflow-runtime.ts` as the repo-local runtime entry for `workflow:health`, `workflow:manifest`, and `workflow:sync`. The export manifest now includes a machine-readable `package_json_contract` describing the minimum `workflow:*`, `gen:*`, and `validate:*` scripts plus runtime dependency requirements for Adoption `A1`, so target projects can merge the contract without undocumented repository knowledge. Host sync remains isolated to `workflow-system-*` namespaces and now converges that namespace on `--write` by pruning orphaned workflow-system skill directories without touching native `gstack-*` artifacts; sync results now distinguish planned prune targets from successfully applied prune targets. `test:workflow-all` now includes `test:workflow-runtime`.
- P11 review follow-up is resolved at the incubation layer: `WORKFLOW_PROTOCOL.md` §18 formalizes lifecycle-governance homes for `ROADMAP.md`, `BASELINES.md`, and `DECISIONS.md`; `scripts/workflow-doc-contracts.ts` and `scripts/run-validation.ts` now enforce lifecycle governance homes for those docs at project-validation time, with additional baseline-slot validation when optional non-functional project gates are bound; synthesized governance-home failures now honor `--blocker-level` by deriving only from project entrypoints that remain in scope after threshold filtering; and `ARCHITECTURE.md` now defines the standalone ownership model used to satisfy the frozen extraction-planning dependency. Target-project-specific command binding, production release automation, and environment procedures remain target-project-owned rather than workflow-system protocol obligations.
