...
function validateSourceNamespace(contract: UnknownRecord): void {
...
function validateRuntimeCatalog(contract: UnknownRecord): Map<string, UnknownRecord> {
  ...
  for (const [index, rawOperation] of rawOperations.entries()) {
    ...
    expectExactKeys(
      operation,
      ['id', 'status', 'binding', 'implementation_phase', 'source_targets', 'write_targets', 'allowed_callers'],
...
export function validateVNextSource(root = resolveRoot()): VNextSourceValidationResult {
  ...
  validateSourceNamespace(sourceContract);
...
if (import.meta.main) {
  try {
    ...
    console.log(
      `vNext source contract: PASSED (${result.entries.length} entries, ${result.capabilities.length} capabilities, ${result.runtimeOperations.length} Runtime operations)`,
    );
  } catch (error) {
