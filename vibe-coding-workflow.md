...
每类文档在使用时应让读者理解它的治理意图、使用边界和何时需要回到规范源确认；正式字段、章节和校验规则不在本文定义。
...
传播治理风险线索只表示“影响可能跨对象、跨模块、跨层或触达受保护边界”。正式触发条件、对象类别、记录要求和 gate 行为以 `WORKFLOW_PROTOCOL.md` / `FILE_SCHEMAS.md` 为准。
...
正式审查输出结构、错误码、blocker、gate 和记录要求以 `WORKFLOW_PROTOCOL.md` / `FILE_SCHEMAS.md` 为准。
...
交付沉淀应覆盖目标、结果、范围、验证和剩余风险；正式 `TASK_SUMMARY.md` / `TASK_ARCHIVE.md` 结构以 `FILE_SCHEMAS.md` 与 `templates/docs/**` 为准。本文不提供可复制摘要模板。
