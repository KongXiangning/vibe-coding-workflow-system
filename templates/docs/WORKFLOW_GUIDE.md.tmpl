...
| `docs/workflow/` | workflow 治理文档、目录索引、registry 与 source-repo reference render | `DOCUMENT_CATALOG.md`、`WORKFLOW_GUIDE.md`、`ROADMAP.md`、`STATUS.md`、`generated/**` |
...
| 文档 | 作用 | 主要使用时机 |
| --- | --- | --- |
| `DOCUMENT_CATALOG.md` | 文档目录协议、分类规则和快速查表入口 | 不确定文档该去哪里读写时 |
...
| --- | --- | --- | --- |
| 新项目设计基线 | `/design-baseline-init` | 原始需求、用户目标、技术栈偏好、交付约束 | `ROADMAP.md`、`docs/designs/architecture.md`、`docs/designs/database.md`、`docs/designs/**`、`BASELINES.md` / `DECISIONS.md` 草案 |
...
| 老项目事实盘点 | `/legacy-inventory` | 由 profile / 仓库事实确定的代码目录、README/docs、package scripts、数据库和部署线索 | `docs/adoption/architecture-inventory.md`、`docs/adoption/database-inventory.md`、`docs/adoption/**`、`ROADMAP.md` 缺口草案 |
...
| 执行当前任务编排 | `/execute-current-task` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`STATUS.md` | 自动进入 review-current-task → lock-scope → classify-decisions → plan-implementation → decompose-task → implement/review/regression 链 |
| 创建任务包 | `/create-current-task` | `.workflow-system/PROJECT_PROFILE.yaml`、`CONTRACTS.md`、`STATUS.md`、`DECISIONS.md`、用户需求 | `CURRENT_TASK.md` |
| 替代失效任务包 | `/supersede-current-task` | 旧 `CURRENT_TASK.md`、`STATUS.md`、`DECISIONS.md`、`CONTRACTS.md` | 当未完成任务包发生 scope invalidation 时，保留旧任务状态、未完成项和 partial diff 归属，并把新任务包送回 `review-current-task` |
| 记录无关新事项 | `/capture-work-item` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`TASKS/inbox/**` | 将与当前任务无关的新事项写入 `TASKS/inbox/**` 的 record-only branch；成功后回到 `/ask-user`，不自动创建任务 |
...
| 分析实现方案 | `/plan-implementation` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`LESSONS.md`、`.workflow-system/PROJECT_PROFILE.yaml` | 架构影响、技术路线、候选方案、风险、兼容性和验证策略 |
| 拆解步骤 | `/decompose-task` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md` | 小步实施计划 |
| 继续当前步骤编排 | `/continue-current-step` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`LESSONS.md` | implement-current-step → review-diff → review-implementation → verify-contracts → run-regression → sync-current-task |
| 调试并修复编排 | `/debug-and-fix-current-task` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`LESSONS.md` | investigate-root-cause → plan-implementation → decompose-task → implement-current-step → review / verify / regression |
...
| 暂停当前任务 | `/pause-current-task` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md` | paused package、fail-closed transaction、resume gate marker |
| 中断当前任务 | `/interrupt-current-task` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md` | interrupted package、checkpoint / dirty / environment / recovery evidence、fail-closed transaction |
| 恢复已暂停任务 | `/resume-paused-task` | `CURRENT_TASK.md`、`TASKS/paused/**`、`CONTRACTS.md`、`DECISIONS.md` | 从完整 payload 重建的 `CURRENT_TASK.md`，并固定回到 `review-current-task` |
| 恢复已中断任务 | `/resume-interrupted-task` | `CURRENT_TASK.md`、`TASKS/interrupted/**`、`CONTRACTS.md`、`DECISIONS.md` | 从完整 payload 重建的 `CURRENT_TASK.md`，并固定回到 `review-current-task` |
| 只读审查编排 | `/review-current-diff` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`LESSONS.md`、`.workflow-system/PROJECT_PROFILE.yaml` | review-diff → review-implementation → verify-contracts → run-regression(report-only) |
| 审查 diff | `/review-diff` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`，以及明确的 diff review target | scope drift / decision drift / safety boundary review / 回归风险 |
| 审查实现质量 | `/review-implementation` | `CURRENT_TASK.md`、`CONTRACTS.md`、`DECISIONS.md`、`LESSONS.md`，以及同一 diff review target 和实现上下文 | Goal fit、Correctness、Edge cases、Robustness、Compatibility、Test adequacy、Remaining risks |
| 同步审查问题 | `/sync-review-findings` | `CURRENT_TASK.md`、结构化 review findings | `CURRENT_TASK.md > 审查问题队列` |
| 验证契约 | `/verify-contracts` | `CONTRACTS.md`、`CURRENT_TASK.md`，以及同一 diff review target | 接口和架构契约检查结果 |
...
| 收尾当前任务编排 | `/close-current-task` | `CURRENT_TASK.md`、`STATUS.md`、`CONTRACTS.md`、`DECISIONS.md`、`LESSONS.md` | sync-current-task → sync-status → optional sync → capture-lessons → prepare-delivery-summary → archive-task |
...
1. 先读 `DOCUMENT_CATALOG.md` 确认文档目录协议，再读 `STATUS.md` / `ROADMAP.md`，确认当前任务是否处在正确窗口。
2. 用 `/create-current-task` 创建 `CURRENT_TASK.md`。
   - 如果未完成的 `CURRENT_TASK.md` 在实现、审查或回归中发现目标、范围锁或验收标准已经失效，先用 `/supersede-current-task` 替代任务包，再重新进入 `/review-current-task` → `/lock-scope` → `/plan-implementation`；不得直接继续 `/implement-current-step`。
...
11. 需要恢复时，用 `/resume-paused-task` 或 `/resume-interrupted-task` 从完整 payload 重建 `CURRENT_TASK.md`；恢复成功后必须回到 `/review-current-task`，不得直接进入 `/implement-current-step`。
...
| Bug 调查并修复 | `/debug-and-fix-current-task` | `/investigate-root-cause` → `/plan-implementation` → `/decompose-task` → `/implement-current-step` → `/review-diff` → `/review-implementation` → `/verify-contracts` → `/run-regression` | 无 CURRENT_TASK.md、根因不收敛、最小修复越界、需要行为/契约确认 |
...
- `/review-diff` 或 `/review-implementation` 的 current-scope mechanical findings 必须先交给 `/sync-review-findings`，由它写入 `CURRENT_TASK.md > 审查问题队列`，然后才能回到 `/implement-current-step`；涉及 User challenge、行为/契约/架构改变或扩大 scope 的 finding 必须停到 `/ask-user` 或 `/lock-scope`。
...
适用于用户提出新功能、改造、体验优化或非 bug 类需求，且当前没有可直接执行的 `CURRENT_TASK.md`。

| 顺序 | Skill | 目的 | 影响文档 | 是否需要人工确认 |
|---:|---|---|---|---|
| 1 | `/create-current-task` | 把需求写成任务包初稿 | `CURRENT_TASK.md` | 需求模糊、目标不清时需要 |
| 1a | `/capture-work-item` | 当当前任务执行中出现无关新事项时，走 record-only branch，只写 `TASKS/inbox/**`，不改当前 `CURRENT_TASK.md` | `TASKS/inbox/**` | relation 不明确、疑似 duplicate 或需要改当前范围时需要 |
| 2 | `/review-current-task` | 收敛目标、验收标准、风险、范围 | `CURRENT_TASK.md` | Taste / User challenge 需要 |
| 3 | `/lock-scope` | 锁定 Allowed / Forbidden / Conditional Files | `CURRENT_TASK.md` | 扩大范围、高风险文件需要 |
| 4 | `/classify-decisions` | 判断哪些方案决策可自动做，哪些要用户拍板 | `CURRENT_TASK.md`、`DECISIONS.md` | Taste / User challenge 必须 |
| 5 | `/plan-implementation` | 分析架构影响、技术路线、候选方案、风险和验证策略 | `CURRENT_TASK.md` | 方案影响、技术取舍或验证策略不清时需要 |
| 6 | `/decompose-task` | 把确认后的方案拆成小步骤 | `CURRENT_TASK.md` | 步骤边界不清时需要 |
| 7 | `/implement-current-step` | 只实现当前步骤 | 代码文件、`CURRENT_TASK.md` | 触碰 forbidden / dangerous surface 需要 |
...
| 12 | `/sync-current-task` | 写回执行结果和验证事实 | `CURRENT_TASK.md` | 事实不明确时需要 |
| 13 | `/sync-decisions` | 沉淀长期有效决策 | `DECISIONS.md` | 只有用户确认或长期决策才写 |
| 14 | `/sync-contracts` | 沉淀稳定接口、架构、行为契约 | `CONTRACTS.md` | 契约语义变化时需要 |
| 15 | `/capture-lessons` | 记录可复用经验 | `LESSONS.md` | 经验是否长期有效不清时需要 |
| 16 | `/prepare-delivery-summary` | 整理交付摘要 | 通常只读 / 交付输出 | 一般不需要 |
| 17 | `/archive-task` | 归档任务并清理当前入口 | `TASKS/...`、`CURRENT_TASK.md` | 任务是否完成不清时需要 |
...
| 顺序 | Skill | 目的 | 影响文档 | 是否需要人工确认 |
|---:|---|---|---|---|
| 1 | `/create-current-task` | 登记 bug，生成 bug 任务包 | `CURRENT_TASK.md` | 复现信息不足时需要 |
| 2 | `/review-current-task` | 收敛 bug 描述、验收、范围、回归检查 | `CURRENT_TASK.md` | bug 边界不清时需要 |
| 3 | `/lock-scope` | 限定调查和修复范围 | `CURRENT_TASK.md` | 需要扩大范围时需要 |
| 4 | `/investigate-root-cause` | 复现问题、收集证据、确认 root cause 和最小修复路径 | `CURRENT_TASK.md` | 根因不收敛、范围外系统可疑时需要 |
| 5 | `/plan-implementation` | 基于 root cause 分析最小修复方案、架构影响、兼容性和验证策略 | `CURRENT_TASK.md` | 修复方案、技术取舍或验证策略不清时需要 |
| 6 | `/decompose-task` | 把修复方案拆成可执行小步骤 | `CURRENT_TASK.md` | 步骤或验证边界不清时需要 |
| 7 | `/implement-current-step` | 执行最小修复 | 代码文件、`CURRENT_TASK.md` | 危险操作、越界修改需要 |
...
| 12 | `/sync-current-task` | 写回根因、修复、验证事实 | `CURRENT_TASK.md` | 事实不明确时需要 |
| 13 | `/sync-contracts` | 记录被明确后的稳定边界 | `CONTRACTS.md` | 契约变化时需要 |
| 14 | `/capture-lessons` | 记录 bug 暴露的可复用经验 | `LESSONS.md` | 经验是否可复用不清时需要 |
| 15 | `/prepare-delivery-summary` | 输出修复摘要 | 通常只读 / 交付输出 | 一般不需要 |
| 16 | `/archive-task` | 归档 bug 任务 | `TASKS/...`、`CURRENT_TASK.md` | 是否完成不清时需要 |
...
| 已有 CURRENT_TASK.md，希望自动推进完整任务 | `/execute-current-task` |
...
| 长任务已经做了 checkpoint commit，需要继续审查 | 在 `CURRENT_TASK.md > 回滚点` 记录 task base / checkpoint，然后用 `/review-diff` 审查 `git diff <base>..HEAD` 或 `git diff <last-reviewed-checkpoint>..HEAD` |
...
- `/review-diff` 或 `/review-implementation` 输出的 mechanical implementation findings 先用 `/sync-review-findings` 写入 `CURRENT_TASK.md > 审查问题队列`，再修复。
