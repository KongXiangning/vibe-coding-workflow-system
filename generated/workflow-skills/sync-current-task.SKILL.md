...
version: 0.2.0
description: >
  Update CURRENT_TASK.md so execution progress, validation status, and remaining
  risks are accurately recorded.
purpose: |
  回写 CURRENT_TASK.md 的执行状态、验证结果和剩余问题。
stage: 阶段 7：状态同步
trigger: |
  每轮实现与验证完成后。
inputs:
...
benefits-from:
  - /run-regression
notes:
  - CURRENT_TASK.md 是任务真实执行记录，不是对外汇报稿。
sync_rules:
  - 每轮结束都要同步
  - 任务偏差和未解决风险必须保留
---
