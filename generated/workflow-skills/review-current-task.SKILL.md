...
version: 0.2.0
description: >
  Review and refine CURRENT_TASK.md so it becomes executable, bounded, and
  auditable.
purpose: |
  审查 CURRENT_TASK.md 初稿并收敛成可执行任务包。
stage: 阶段 1：需求进入
trigger: |
  当 CURRENT_TASK.md 初稿已经生成，进入实现前。
inputs:
  - current_task_draft
  - contracts
  - confirmed_decisions
