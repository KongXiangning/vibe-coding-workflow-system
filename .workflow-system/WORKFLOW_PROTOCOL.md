...
The Phase 0 Runtime canonical-source allowlist is closed to the five live governance documents (`CURRENT_TASK`, `STATUS`, `CONTRACTS`, `DECISIONS`, `LESSONS`), `AGENTS.md`, bounded `TASKS/paused`, `TASKS/interrupted`, `TASKS/inbox`, and the materialized canonical task archive path. The write-target allowlist uses the same exact documents plus bounded family patterns for paused / interrupted / inbox artifacts. Existing `CLAUDE.md` files are target-owned legacy guidance and are not a current workflow-system source or write target. Broad sources or targets such as `TASKS`, `**`, `docs/**`, or `TASKS/**` are invalid.
...
references, an idempotency key, and exactly one requested write target. The
kernel rejects legacy or unsupported CURRENT_TASK schemas with
`migration-required`, rejects stale source tuples and wrong operation paths,
and never lets one operation borrow another operation's write boundary.
